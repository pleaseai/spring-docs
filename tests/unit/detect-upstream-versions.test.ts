...
describe('parseArgs', () => {
  test('defaults to every supported project, no limit, and text output', () => {
    ...
    expect(args).toEqual({ projects: ['ai', 'boot', 'framework', 'security'], limit: null, json: false })
  })
  ...
  test('accepts --limit and --json', () => {
    ...
    expect(args).toEqual({ projects: ['ai', 'boot', 'framework', 'security'], limit: 5, json: true })
  })
