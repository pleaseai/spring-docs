...
source: "ROOT:redis/redis-repositories/anatomy.adoc"
---

<a id="redis.repositories.anatomy"></a>
...
<a id="redis.repositories.anatomy.insert"></a>
...
1. Add the key of the hash written in <1> to the helper index of entities in the same keyspace.
1. Add the key of the hash written in <2> to the secondary index of firstnames with the properties value.
1. Add the index of <3> to the set of helper structures for entry to keep track of indexes to clean on delete/update.

<a id="redis.repositories.anatomy.replace"></a>
...
1. Save the flattened entry as hash.
1. Add the key of the hash written in <1> to the helper index of entities in the same keyspace.
1. Get existing index structures that might need to be updated.
1. Check if the index exists and what type it is (text, geo, …).
1. Remove a potentially existing key from the index.
1. Remove the helper holding index information.
1. Add the key of the hash added in <2> to the secondary index of firstnames with the properties value.
1. Add the index of <6> to the set of helper structures for entry to keep track of indexes to clean on delete/update.

<a id="redis.repositories.anatomy.geo"></a>
...
1. Keep track of the index structure.

<a id="redis.repositories.anatomy.index"></a>
...
1. Fetch each key returned by <1> individually.

<a id="redis.repositories.anatomy.geo-index"></a>
