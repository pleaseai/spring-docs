...
> [!NOTE]
> When using Spring Boot, `LettuceMetricsAutoConfiguration` configures Lettuce’s `MicrometerCommandLatencyRecorder`.
...
> [!NOTE]
> Use Lettuce’s built-in `MicrometerTracing` as `MicrometerTracingAdapter` has been deprecated for removal in future releases.
