---
title: "Redis Streams"
source: "ROOT:redis/redis-streams.adoc"
---
...
Although this pattern has similarities to [Pub/Sub](pubsub.md), the main difference lies in the persistence of messages and how they are consumed.
...
<a id="redis.streams.receive.synchronous"></a>
...
<a id="redis.streams.receive.containers"></a>
...
- [`StreamMessageListenerContainer`](https://docs.spring.io/spring-data/redis/docs/3.5.12/api/org/springframework/data/redis/stream/StreamMessageListenerContainer.html) acts as message listener container for imperative programming models. It is used to consume records from a Redis Stream and drive the [`StreamListener`](https://docs.spring.io/spring-data/redis/docs/3.5.12/api/org/springframework/data/redis/stream/StreamListener.html) instances that are injected into it.
- [`StreamReceiver`](https://docs.spring.io/spring-data/redis/docs/3.5.12/api/org/springframework/data/redis/stream/StreamReceiver.html) provides a reactive variant of a message listener. It is used to consume messages from a Redis Stream as potentially infinite stream and emit stream messages through a `Flux`.
...
<a id="imperative-streammessagelistenercontainer"></a>

#### Imperative `StreamMessageListenerContainer`

In a fashion similar to a Message-Driven Bean (MDB) in the EJB world, the Stream-Driven POJO (SDP) acts as a receiver for Stream messages. The one restriction on an SDP is that it must implement the [`StreamListener`](https://docs.spring.io/spring-data/redis/docs/3.5.12/api/org/springframework/data/redis/stream/StreamListener.html) interface. Please also be aware that in the case where your POJO receives messages on multiple threads, it is important to ensure that your implementation is thread-safe.
...
<a id="reactive-streamreceiver"></a>
...
<a id="redis.streams.acknowledge"></a>
...
1. Acknowledged the message after processing.

> [!TIP]
> To auto acknowledge messages on receive use `receiveAutoAck` instead of `receive`.

<a id="redis.streams.receive.readoffset"></a>
...
<a id="redis.streams.receive.serialization"></a>
...
<a id="redis.streams.hashing"></a>

## Object Mapping

<a id="simple-values"></a>
...
- Convert the value into a `Map` suitable for serialization using a [`HashMapper`](hash-mappers.md).
...
> [!NOTE]
> [HashMappers](hash-mappers.md) convert the payload to a `Map` with specific types. Make sure to use Hash-Key and Hash-Value serializers that are capable of (de-)serializing the hash.
...
`StreamOperations` use by default [ObjectHashMapper](redis-repositories/mapping.md).
