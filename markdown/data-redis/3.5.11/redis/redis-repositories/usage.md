---
title: "Usage"
source: "ROOT:redis/redis-repositories/usage.adoc"
---

<a id="redis.repositories.usage"></a>
...
1. Uses the provided `id` to retrieve the object stored at `keyspace:id`.
1. Counts the total number of entities available within the keyspace, `people`, defined by `@RedisHash` on `Person`.
1. Removes the key for the given object from Redis.

<a id="redis.repositories.references"></a>
...
<a id="redis.repositories.partial-updates"></a>
...
1. Set the simple 'address.city' property to 'emond’s field' without having to pass in the entire object.
This does not work when a custom conversion is registered.
1. Remove the `age` property.
1. Set complex `address` property.
1. Set a map of values, which removes the previously existing map and replaces the values with the given ones.
1. Automatically update the server expiration time when altering [Time To Live](expirations.md).
