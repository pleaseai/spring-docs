---
title: "Saving, Updating, and Removing Documents"
source: "ROOT:mongodb/template-crud-operations.adoc"
---

<a id="mongo-template.save-update-remove"></a>
...
The query syntax used in the preceding example is explained in more detail in the section “[Querying Documents](template-query-operations.md)”.
...
> [!IMPORTANT]
> MongoDB collections can contain documents that represent instances of a variety of types. Please refer to the [type mapping](converters-type-mapping.md) for details.

<a id="mongo-template.save-insert"></a>
...
<a id="mongo-template.id-handling"></a>
...
1. A property or field without an annotation but named `id` maps to the `_id` field.
...
1. An `id` property or field declared as `Date` is converted to and stored as `ObjectId`.
1. An `id` property or field declared as `BigInteger` in the Java class is converted to and stored as an `ObjectId` by using a Spring `Converter<BigInteger, ObjectId>`.
...
1. The id is treated as `ObjectId`.
1. The id is treated as `ObjectId` if the given `String` is a valid `ObjectId` hex, otherwise as `String`. Corresponds to `@Id` usage.

<a id="mongo-template.save-insert.collection"></a>
...
<a id="mongo-template.save-insert.individual"></a>
...
<a id="mongo-template.save-insert.batch"></a>
...
> However bulk operations do not publish [lifecycle events](lifecycle-events.md).
...
>
> Read more in the see [Optimistic Locking](#mongo-template.optimistic-locking) section.

<a id="mongodb-template-update"></a>
...
<a id="mongodb-template-update.methods"></a>
...
> [!WARNING]
> `updateFirst` does not support ordering for MongoDB Versions below 8.0. Running one of the older versions, please use [findAndModify](#mongo-template.find-and-upsert) to apply `Sort`.

> [!NOTE]
> Index hints for the update operation can be provided via `Query.withHint(…​)`.

<a id="mongodb-template-update.update"></a>
...
<a id="mongo-template.aggregation-update"></a>

### Aggregation Pipeline Updates

Update methods exposed by `MongoOperations` and `ReactiveMongoOperations` also accept an [Aggregation Pipeline](aggregation-framework.md) via `AggregationUpdate`.
...
1. The 2nd `$set` stage calculates a new field *grade* based on the *average* field calculated by the first aggregation stage.
1. The pipeline is run on the *students* collection and uses `Student` for the aggregation field mapping.
1. Apply the update to all matching documents in the collection.

<a id="mongo-template.upserts"></a>
...
> [!WARNING]
> `upsert` does not support ordering. Please use [findAndModify](#mongo-template.find-and-upsert) to apply `Sort`.

> [!IMPORTANT]
> `@Version` properties if not included in the `Update` will be automatically initialized.
> Read more in the see [Optimistic Locking](#mongo-template.optimistic-locking) section.

<a id="mongo-template.replace"></a>
...
1. The query used to identify the single document to replace.
1. Set up the replacement document which must hold either the same `_id` as the existing or no `_id` at all.
...
1. Use `upsert` to insert a new document if no match is found
...
<a id="mongo-template.find-and-upsert"></a>
...
<a id="mongo-template.find-and-replace"></a>
...
1. The actual match query mapped against the given domain type. Provide `sort`, `fields` and `collation` settings via the query.
1. Additional optional hook to provide options other than the defaults, like `upsert`.
1. An optional projection type used for mapping the operation result. If none given the initial domain type is used.
1. Trigger the actual processing. Use `findAndReplaceValue` to obtain the nullable result instead of an `Optional`.
...
1. Remove all documents that match the criteria of the query from the `GOT` collection.
1. Remove the first three documents in the `GOT` collection. Unlike <2>, the documents to remove are identified by their `_id`, running the given query, applying `sort`, `limit`, and `skip` options first, and then removing all at once in a separate step.
1. Remove all documents matching the criteria of the query from the `GOT` collection. Unlike <3>, documents do not get deleted in a batch but one by one.
1. Remove the first three documents in the `GOT` collection. Unlike <3>, documents do not get deleted in a batch but one by one.

<a id="mongo-template.optimistic-locking"></a>
...
1. Update the document with `version = 0`. Set the `lastname` and bump `version` to `1`.
...
> [!NOTE]
> Optimistic Locking requires write acknowledgement (a write result response) by the server. Using `WriteConcern.UNACKNOWLEDGED` can lead to silently swallowed `OptimisticLockingFailureException`.
