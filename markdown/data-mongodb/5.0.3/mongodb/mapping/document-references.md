---
title: "Using DBRefs"
source: "ROOT:mongodb/mapping/document-references.adoc"
---

<a id="mapping-usage-references"></a>
...
<a id="mapping-usage.document-references"></a>
...
1. The mapping framework does not handle cascading saves, so make sure to persist the referenced entity individually.
1. Add the reference to the existing entity.
1. Referenced `Account` entities are represented as an array of their `_id` values.
...
1. Lazy load back references to the `Book` collection.
...
1. Use the `acronym` field to query for entities in the `Publisher` collection.
1. The field value placeholders of the lookup query (like `acc`) is used to form the reference document.
...
1. Mark the property holding the references to be readonly.
This prevents storing references to individual `Book`s with the `Publisher` document.
1. Use the `#self` variable to access values within the `Publisher` document and in this retrieve `Books` with matching `publisherId`.
...
1. `refKey` is short for `target.refKey`.
...
1. Use non *id* fields for the lookup of the target documents.
...
1. The collection name can be read from the reference document using its key.
