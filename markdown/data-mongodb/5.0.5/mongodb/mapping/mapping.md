...
> In all other cases you can exclude synthetic fields for delegated properties by annotating the property with [`@Transient`](https://docs.spring.io/spring-data/commons/reference/4.0/api/java/org/springframework/data/annotation/Transient.html).
...
| `java.util.UUID` (According to UuidRepresentation) | native | `{"uuid" : { "$binary" : "MEaf1CFQ6lSphaa3b9AtlA==", "$type" : "04" }}` |
...
| `BigInteger` | native `NumberDecimal`, `String` (see `BigDecimalRepresentation`) | `{"value" : NumberDecimal(741) }`, `{"value" : "741" }` |
| `BigDecimal` | native `NumberDecimal`, `String` (see `BigDecimalRepresentation`) | `{"value" : NumberDecimal(741.99) }`, `{"value" : "741.99" }` |
...
| `LocalDate` (Java 8) | converter / native (Java 8)<sup>\[[1](#_footnotedef_1)\]</sup> | `{"date" : ISODate("2019-11-12T00:00:00.000Z")}` |
| `LocalDateTime`, `LocalTime` (Java 8) | converter / native (Java 8)<sup>\[[2](#_footnotedef_2)\]</sup> | `{"date" : ISODate("2019-11-12T23:00:00.809Z")}` |
