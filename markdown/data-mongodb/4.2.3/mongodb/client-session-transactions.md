...
source: "ROOT:mongodb/client-session-transactions.adoc"
---

<a id="mongo.sessions"></a>
...
<a id="mongo.sessions.reactive"></a>
...
1. Use `MongoOperation` methods as before. The `ClientSession` gets applied automatically.
1. Make sure to close the `ClientSession`.
1. Close the session.
...
1. Use `ReactiveMongoOperation` methods as before. The `ClientSession` is obtained and applied automatically.
1. Make sure to close the `ClientSession`.
1. Nothing happens until you subscribe. See [the Project Reactor Reference Guide](https://projectreactor.io/docs/core/release/reference/#reactive.subscribe) for details.
...
<a id="mongo.transactions"></a>

## MongoDB Transactions

As of version 4, MongoDB supports [Transactions](https://www.mongodb.com/transactions). Transactions are built on top of [Sessions](#) and, consequently, require an active `ClientSession`.
...
1. If everything works out as expected, commit the changes.
1. Something broke, so roll back everything.
1. Do not forget to close the session when done.
...
1. Once we have the `ClientSession` at hand, start the transaction.
1. Operate within the transaction by passing on the `ClientSession` to the operation.
1. If the operations completes exceptionally, we need to stop the transaction and preserve the error.
1. Or of course, commit the changes in case of success. Still preserving the operations result.
1. Lastly, we need to make sure to close the session.
...
<a id="mongo.transactions.reactive-operator"></a>
...
1. Create the `TransactionTemplate` using the provided `PlatformTransactionManager`.
1. Within the callback the `ClientSession` and transaction are already registered.
...
1. Create the `TransactionalOperator` using the provided `ReactiveTransactionManager`.
1. `TransactionalOperator.transactional(…)` provides transaction management for all upstream operations.

<a id="mongo.transactions.reactive-tx-manager"></a>
...
It lets applications use [the managed transaction features of Spring](https://docs.spring.io/spring-framework/reference/6.1/data-access.html#transaction).
...
1. Mark methods as transactional.
...
<a id="mongo.transactions.behavior"></a>
...
> [!NOTE]
> Make sure to add `replicaSet` to the MongoDB URI. Please refer to [connection string options](https://docs.mongodb.com/manual/reference/connection-string/#connections-connection-options) for further details.
...
We highly recommend [Spring Retry](https://github.com/spring-projects/spring-retry) for those purposes. Nevertheless
one may override `MongoTransactionManager#doCommit(MongoTransactionObject)` to implement a [Retry Commit Operation](https://docs.mongodb.com/manual/core/transactions/#retry-commit-operation)
