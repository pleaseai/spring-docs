...
source: "ROOT:mongodb/aggregation-framework.adoc"
---

<a id="mongo.aggregation"></a>
...
<a id="mongo.aggregation.basic-concepts"></a>

## Basic Concepts

The Aggregation Framework support in Spring Data MongoDB is based on the following key abstractions: [`Aggregation`](https://docs.spring.io/spring-data/mongodb/docs/5.0.6/api/org/springframework/data/mongodb/core/aggregation/Aggregation.html) and [`AggregationResults`](https://docs.spring.io/spring-data/mongodb/docs/5.0.6/api/org/springframework/data/mongodb/core/aggregation/AggregationResults.html).
...
| Group/Accumulator Aggregation Operators | `addToSet`, `bottom`, `bottomN`, `covariancePop`, `covarianceSamp`, `expMovingAvg`, `first`, `firstN`, `last`, `lastN` `max`, `maxN`, `min`, `minN`, `avg`, `push`, `sum`, `top`, `topN`, `count` (\*), `median`, `percentile`, `stdDevPop`, `stdDevSamp` |
...
<a id="mongo.aggregation.projection"></a>
...
<a id="mongo.aggregation.facet"></a>
...
To create output fields in buckets, bucket operations can use `AggregationExpression` through `andOutput()` and [SpEL expressions](#mongo.aggregation.projection.expressions) through `andOutputExpression()`.
...
<a id="multi-faceted-aggregation"></a>
...
<a id="mongo.aggregation.sort-by-count"></a>
...
<a id="mongo.aggregation.projection.expressions"></a>
...
<a id="complex-calculations-with-spel-expressions"></a>
...
You can see examples in more context in [Aggregation Framework Example 5](#mongo.aggregation.examples.example5) and [Aggregation Framework Example 6](#mongo.aggregation.examples.example6).
...
<a id="mongo.aggregation.examples"></a>
...
<a id="mongo.aggregation.examples.example1"></a>
...
1. Use the `project` operation to select the `tags` field (which is an array of strings) from the input collection.
1. Use the `unwind` operation to generate a new document for each tag within the `tags` array.
1. Use the `group` operation to define a group for each `tags` value for which we aggregate the occurrence count (by using the `count` aggregation operator and collecting the result in a new field called `n`).
1. Select the `n` field and create an alias for the ID field generated from the previous group operation (hence the call to `previousOperation()`) with a name of `tag`.
1. Use the `sort` operation to sort the resulting list of tags by their occurrence count in descending order.
1. Call the `aggregate` method on `MongoTemplate` to let MongoDB perform the actual aggregation operation, with the created `Aggregation` as an argument.
...
<a id="mongo.aggregation.examples.example2"></a>
...
1. Use the `sort` operation to sort the intermediate-result by the `pop`, `state` and `city` fields, in ascending order, such that the smallest city is at the top and the biggest city is at the bottom of the result. Note that the sorting on `state` and `city` is implicitly performed against the group ID fields (which Spring Data MongoDB handled).
1. Use a `group` operation again to group the intermediate result by `state`. Note that `state` again implicitly references a group ID field. We select the name and the population count of the biggest and smallest city with calls to the `last(…)` and `first(…​)` operators, respectively, in the `project` operation.
1. Select the `state` field from the previous `group` operation. Note that `state` again implicitly references a group ID field. Because we do not want an implicitly generated ID to appear, we exclude the ID from the previous operation by using `and(previousOperation()).exclude()`. Because we want to populate the nested `City` structures in our output class, we have to emit appropriate sub-documents by using the nested method.
1. Sort the resulting list of `StateStats` by their state name in ascending order in the `sort` operation.
...
<a id="mongo.aggregation.examples.example3"></a>
...
1. Sort the intermediate result by the id-reference of the previous group operation in addition to the `"totalPop"` field in ascending order.
1. Filter the intermediate result by using a `match` operation which accepts a `Criteria` query as an argument.
...
<a id="mongo.aggregation.examples.example4"></a>
...
<a id="mongo.aggregation.examples.example5"></a>
...
<a id="mongo.aggregation.examples.example6"></a>
...
<a id="mongo.aggregation.examples.example7"></a>
...
1. does not contain a value,
1. then use [`$$REMOVE`](https://docs.mongodb.com/manual/reference/aggregation-variables/#variable.REMOVE) to exclude the field.
1. Otherwise, add the field value of `author.middle`.
