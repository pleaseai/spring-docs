...
<a id="mongo-template.convenience-methods"></a>

## Convenience Methods

The [`MongoTemplate`](https://docs.spring.io/spring-data/mongodb/docs/4.5.12/api/org/springframework/data/mongodb/core/MongoTemplate.html) class implements the interface [`MongoOperations`](https://docs.spring.io/spring-data/mongodb/docs/4.5.12/api/org/springframework/data/mongodb/core/MongoOperations.html).
...
For more information please refer to the [CRUD](template-crud-operations.md) and [Query](template-query-operations.md) sections of the documentation.

> [!NOTE]
> The preferred way to reference the operations on `MongoTemplate` instance is through its interface, `MongoOperations`.

<a id="mongo-template.execute-callbacks"></a>
...
The following example uses the [`CollectionCallback`](https://docs.spring.io/spring-data/mongodb/docs/4.5.12/api/org/springframework/data/mongodb/core/CollectionCallback.html) to return information about an index:
...
<a id="mongo-template.fluent-api"></a>
...
1. The collection name to use if not defined on the domain type.
1. Result type if not using the original domain type.
1. The lookup query.
...
> This applies as long as the [`Query`](https://docs.spring.io/spring-data/mongodb/docs/4.5.12/api/org/springframework/data/mongodb/core/query/Query.html) itself does not contain any field restriction and the target type is a closed interface or DTO projection.

> [!WARNING]
> Projections must not be applied to [DBRefs](mapping/document-references.md).
...
<a id="mongo-template.exception-translation"></a>
...
Some of the mappings performed by the [`MongoExceptionTranslator`](https://docs.spring.io/spring-data/mongodb/docs/4.5.12/api/org/springframework/data/mongodb/core/MongoExceptionTranslator.html) are `com.mongodb.Network` to `DataAccessResourceFailureException` and `MongoException` error codes 1003, 12001, 12010, 12011, and 12012 to `InvalidDataAccessApiUsageException`.
Look into the implementation for more details on the mapping.

Exception Translation can be configured by setting a customized [`MongoExceptionTranslator`](https://docs.spring.io/spring-data/mongodb/docs/4.5.12/api/org/springframework/data/mongodb/core/MongoExceptionTranslator.html) on your `MongoDatabaseFactory` or its reactive variant.
...
<a id="mongo-template.type-mapping"></a>

## Domain Type Mapping

The mapping between MongoDB documents and domain classes is done by delegating to an implementation of the [`MongoConverter`](https://docs.spring.io/spring-data/mongodb/docs/4.5.12/api/org/springframework/data/mongodb/core/convert/MongoConverter.html) interface.
Spring provides [`MappingMongoConverter`](https://docs.spring.io/spring-data/mongodb/docs/4.5.12/api/org/springframework/data/mongodb/core/convert/MappingMongoConverter.html), but you can also write your own converter.
...
These conventions, as well as the use of mapping annotations, are explained in the [Mapping](mapping/mapping.md) chapter.
