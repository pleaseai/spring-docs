...
<a id="cassandra.template.query.columns"></a>
...
If you provide more information to a query (e.g. when using closed DTO or Interface projections through [Fluent Template API](#cassandra.template.query.fluent-template-api)) then the column selection will be limited.
...
1. Include a function call to `similarity_cosine(vector, […])`. `Columns.select(…)` uses a selector builder customizer to configure the actual selection.
Make sure to declare an alias to map the result onto `CommentSearch.similarity` as result mapping uses column names.
1. Use the `VectorSort.ann(…)` to define the sort order in comparison to the given vector.
1. Define the target type to map the result to.
