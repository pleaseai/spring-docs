...
Given a `Connection`, the `PreparedStatementCreator` callback interface creates a [prepared statement](prepared-statements.md#cassandra.template.prepared-statements.cql) with the provided CQL and any necessary parameter arguments.
The `RowCallbackHandler` interface extracts values from each row of a `ResultSet`.

The `CqlTemplate` can be used within a DAO implementation through direct instantiation with a `SessionFactory` reference or be configured in the Spring container and given to DAOs as a bean reference. `CqlTemplate` is a foundational building block for [`CassandraTemplate`](template.md).
...
> [`ReactiveCqlTemplate`](#cassandra.reactive.cql-template) for reactive execution.
...
See the [Javadoc](https://docs.spring.io/spring-data/cassandra/docs/4.2.0/api/) for that.
