...
```groovy
ext['spring-security.version']='6.4.12'
```
...
dependencyManagement {
	imports {
		mavenBom 'org.springframework.security:spring-security-bom:6.4.12'
	}
}
```
...
Spring Security builds against Spring Framework 6.2.12 but should generally work with any newer version of Spring Framework 5.x.
Many users are likely to run afoul of the fact that Spring Security’s transitive dependencies resolve Spring Framework 6.2.12, which can cause strange classpath problems.
...
dependencyManagement {
	imports {
		mavenBom 'org.springframework:spring-framework-bom:6.2.12'
	}
}
```
...
The preceding example ensures that all the transitive dependencies of Spring Security use the Spring 6.2.12 modules.
