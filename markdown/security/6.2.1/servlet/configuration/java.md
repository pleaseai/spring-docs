...
The reason for this is that Spring Security needs to be able to inspect some Spring MVC configuration in order to appropriately configure [underlying request matchers](../authorization/authorize-http-requests.md#_request_matchers), so they need to be in the same application context.
