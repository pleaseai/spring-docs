...
Spring Security builds against Spring Framework 6.1.18 but should generally work with any newer version of Spring Framework 5.x.
Many users are likely to run afoul of the fact that Spring Security’s transitive dependencies resolve Spring Framework 6.1.18, which can cause strange classpath problems.
...
The preceding example ensures that all the transitive dependencies of Spring Security use the Spring 6.1.18 modules.
...
```groovy
ext['spring-security.version']='6.3.8'
```
...
dependencyManagement {
	imports {
		mavenBom 'org.springframework.security:spring-security-bom:6.3.8'
	}
}
```
