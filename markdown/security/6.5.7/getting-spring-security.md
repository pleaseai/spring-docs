...
```groovy
ext['spring-security.version']='6.5.7'
```
...
dependencyManagement {
	imports {
		mavenBom 'org.springframework.security:spring-security-bom:6.5.7'
	}
}
```
...
Spring Security builds against Spring Framework 6.2.13 but should generally work with any newer version of Spring Framework 5.x.
Many users are likely to run afoul of the fact that Spring Security’s transitive dependencies resolve Spring Framework 6.2.13, which can cause strange classpath problems.
...
The preceding example ensures that all the transitive dependencies of Spring Security use the Spring 6.2.13 modules.
