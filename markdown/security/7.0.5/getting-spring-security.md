...
Spring Security builds against Spring Framework 7.0.7 but should generally work with any newer version of Spring Framework 5.x.
Many users are likely to run afoul of the fact that Spring Security’s transitive dependencies resolve Spring Framework 7.0.7, which can cause strange classpath problems.
...
dependencyManagement {
	imports {
		mavenBom 'org.springframework:spring-framework-bom:7.0.7'
	}
}
```
...
The preceding example ensures that all the transitive dependencies of Spring Security use the Spring 7.0.7 modules.
