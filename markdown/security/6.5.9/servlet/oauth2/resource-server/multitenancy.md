...
source: "ROOT:servlet/oauth2/resource-server/multitenancy.adoc"
---

# OAuth 2.0 Resource Server Multi-tenancy

<a id="oauth2reourceserver-opaqueandjwt"></a>
...
<a id="oauth2resourceserver-multitenancy"></a>
...
1. Propagate the tenant

<a id="_resolving_the_tenant_by_claim"></a>
...
<a id="_parsing_the_claim_only_once"></a>

#### Parsing the Claim Only Once

You may have observed that this strategy, while simple, comes with the trade-off that the JWT is parsed once by the `AuthenticationManagerResolver` and then again by the [`JwtDecoder`](jwt.md#oauth2resourceserver-jwt-architecture-jwtdecoder) later on in the request.

This extra parsing can be alleviated by configuring the [`JwtDecoder`](jwt.md#oauth2resourceserver-jwt-architecture-jwtdecoder) directly with a `JWTClaimsSetAwareJWSKeySelector` from Nimbus:
...
1. A cache for `JWSKeySelector`s, keyed by tenant identifier
1. Looking up the tenant is more secure than simply calculating the JWK Set endpoint on the fly - the lookup acts as a list of allowed tenants
1. Create a `JWSKeySelector` via the types of keys that come back from the JWK Set endpoint - the lazy lookup here means that you don’t need to configure all tenants at startup
...
Now that we have a tenant-aware processor and a tenant-aware validator, we can proceed with creating our [`JwtDecoder`](jwt.md#oauth2resourceserver-jwt-architecture-jwtdecoder):
...
However, if you resolve it by a claim in the bearer token, read on to learn about [Spring Security’s support for bearer token propagation](bearer-tokens.md#oauth2resourceserver-bearertoken-resolver).
