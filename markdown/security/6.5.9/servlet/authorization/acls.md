---
title: "Domain Object Security (ACLs)"
source: "ROOT:servlet/authorization/acls.adoc"
---
...
<a id="domain-acls-key-concepts"></a>
...
Columns include the ID, a foreign key to the ACL\_CLASS table, a unique identifier so we know the ACL\_CLASS instance for which we provide information, the parent, a foreign key to the ACL\_SID table to represent the owner of the domain object instance, and whether we allow ACL entries to inherit from any parent ACL.
...
Columns include a foreign key to the `ACL_OBJECT_IDENTITY`, the recipient (i.e. a foreign key to ACL\_SID), whether we’ll be auditing or not, and the integer bit mask that represents the actual permission being granted or denied.
...
While you have 32 bits available for permissions, you could have billions of domain object instances (which means billions of rows in ACL\_OBJECT\_IDENTITY and, probably, ACL\_ENTRY).
...
The first is the [Contacts Sample](https://github.com/spring-projects/spring-security-samples/tree/6.5.x/servlet/xml/java/contacts), and the other is the [Document Management System (DMS) Sample](https://github.com/spring-projects/spring-security-samples/tree/6.5.x/servlet/xml/java/dms).
We suggest taking a look at these examples.

<a id="domain-acls-getting-started"></a>
...
See one of the [samples](https://github.com/spring-projects/spring-security-samples/tree/6.5.x) that ship with Spring Security for an example configuration.
You also need to populate the database with the [four ACL-specific tables](#acl_tables) listed in the previous section (see the ACL samples for the appropriate SQL statements).
...
<a id="_using_the_permissionevaluator"></a>
...
Then using [method-based security](method-security.md#authorizing-with-annotations) you can use `hasPermission` in your annotation expressions like so:
