...
By default, the resulting `Authentication#getPrincipal` is a Spring Security `OAuth2AuthenticatedPrincipal` object, and `Authentication#getName` maps to the token’s `sub` property, if one is present.
