...
This is quite simple with `ServletBearerExchangeFilterFunction`, which you can see in the following example:
...
When the above `WebClient` is used to perform requests, Spring Security will look up the current `Authentication` and extract any `AbstractOAuth2Token` credential.
...
> [!NOTE]
> Unlike the [OAuth 2.0 Client filter function](https://docs.spring.io/spring-security/site/docs/6.3.10/api/org/springframework/security/oauth2/client/web/reactive/function/client/ServletOAuth2AuthorizedClientExchangeFilterFunction.html), this filter function makes no attempt to renew the token, should it be expired.
...
> [!NOTE]
> Unlike the [OAuth 2.0 Authorized Client Manager](https://docs.spring.io/spring-security/site/docs/6.3.10/api/org/springframework/security/oauth2/client/OAuth2AuthorizedClientManager.html), this filter interceptor makes no attempt to renew the token, should it be expired.
