...
> [!NOTE]
> Spring Security provides the configuration by using Spring MVC’s [`WebMvcConfigurer`](https://docs.spring.io/spring-framework/reference/7.0.8/web/webmvc/mvc-config/customize.html).
...
If you **are** customizing Spring MVC’s `PathPatternParser` instance, you will need to [configure Spring Security and Spring MVC in the same `ApplicationContext`](#security-mvc-same-application-context).
...
Now that Spring MVC is integrated with Spring Security, you are ready to write some [authorization rules](../authorization/authorize-http-requests.md) that will use `PathPatternRequestMatcher`.
...
We could then access the `CustomUser` by using a [SpEL expression](https://docs.spring.io/spring-framework/reference/7.0.8/core/expressions.html) that uses `Authentication.getPrincipal()` as the root object:
...
<a id="mvc-current-security-context"></a>
...
Spring Web MVC 3.2+ has excellent support for [Asynchronous Request Processing](https://docs.spring.io/spring-framework/reference/7.0.8/web/webmvc/mvc-ann-async.html).
...
Spring Security automatically [include the CSRF Token](../exploits/csrf.md#csrf-integration-form) within forms that use the [Spring MVC form tag](https://docs.spring.io/spring-framework/reference/7.0.8/web/webmvc-view/mvc-jsp.html#mvc-view-jsp-formtaglib-formtag).
...
<a id="security-mvc-same-application-context"></a>
