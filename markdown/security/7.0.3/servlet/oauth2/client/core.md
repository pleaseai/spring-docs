---
title: "Core Interfaces and Classes"
source: "ROOT:servlet/oauth2/client/core.adoc"
---
...
1. `(userInfoEndpoint)uri`: The UserInfo Endpoint URI used to access the claims and attributes of the authenticated end-user.
...
1. <a id="oauth2Client-client-registration-requireProofKey"></a>`requireProofKey`: If `true` or if `clientAuthenticationMethod` is `none`, then PKCE will be enabled. Defaults to `true` for `authorization_code` grant type and `false` for other grant types.
...
> [!NOTE]
> `JdbcOAuth2AuthorizedClientService` depends on the table definition described in [ OAuth 2.0 Client Schema](../../appendix/database-schema.md#dbschema-oauth2-client).
