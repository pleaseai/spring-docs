...
![filterchain](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/servlet/architecture/filterchain.png)
...
In a Spring MVC application, the `Servlet` is an instance of [`DispatcherServlet`](https://docs.spring.io/spring-framework/reference/7.0.9/web.html#mvc-servlet).
...
#### `FilterChain` Usage Example - Java
...
#### `FilterChain` Usage Example - Kotlin
...
Spring provides a `Filter` implementation named [`DelegatingFilterProxy`](https://docs.spring.io/spring-framework/docs/7.0.9/javadoc-api/org/springframework/web/filter/DelegatingFilterProxy.html) that allows bridging between the Servlet container’s lifecycle and Spring’s `ApplicationContext`.
...
![delegatingfilterproxy](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/servlet/architecture/delegatingfilterproxy.png)
...
#### `DelegatingFilterProxy` Pseudo Code - Java
...
#### `DelegatingFilterProxy` Pseudo Code - Kotlin
...
![filterchainproxy](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/servlet/architecture/filterchainproxy.png)

<a id="servlet-securityfilterchain"></a>

## SecurityFilterChain

[`SecurityFilterChain`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/web/SecurityFilterChain.html)  is used by [FilterChainProxy](#servlet-filterchainproxy) to determine which Spring Security `Filter` instances should be invoked for the current request.
...
![securityfilterchain](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/servlet/architecture/securityfilterchain.png)
...
![multi securityfilterchain](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/servlet/architecture/multi-securityfilterchain.png)
...
[exploit protection](exploits/index.md), [authentication](authentication/index.md), [authorization](authorization/index.md), and more.
...
However, there are times that it is beneficial to know the ordering, if you want to know them, you can check the [`FilterOrderRegistration` code](https://github.com/spring-projects/spring-security/tree/7.1.1/config/src/main/java/org/springframework/security/config/annotation/web/builders/FilterOrderRegistration.java).

These security filters are most often declared using an [`HttpSecurity`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/config/annotation/web/builders/HttpSecurity.html) instance.
...
1. Second, [the authentication filters](authentication/architecture.md) are invoked to authenticate the request.
1. Third, [the `AuthorizationFilter`](authorization/authorize-http-requests.md) is invoked to authorize the request.
...
For example, you want to make sure that the [filter you have added](#adding-filters-to-chain) is in the list of the security filters.
...
<a id="adding-filters-to-chain"></a>

### Adding Filters to the Filter Chain

Most of the time, the default [Security Filters](#servlet-security-filters) are enough to provide security to your application.
However, there might be times that you want to add a custom `Filter` to the [SecurityFilterChain](#servlet-securityfilterchain).

[`HttpSecurity`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/config/annotation/web/builders/HttpSecurity.html) comes with three methods for adding filters:
...
1. [`SecurityContext`](authentication/architecture.md#servlet-authentication-securitycontextholder) is loaded from the session
1. Request is protected from common exploits; [secure headers](../features/exploits/headers.md), [CORS](integrations/cors.md), [CSRF](exploits/csrf.md)
1. Request is [authenticated](authentication/architecture.md)
1. Request is [authorized](authorization/architecture.md)
...
> This means they should be placed after [`LogoutFilter`](authentication/logout.md).
...
Now, you need to add the filter to the [SecurityFilterChain](#servlet-securityfilterchain).
...
Based on the rule of thumb, you can add it before [`LogoutFilter`](authentication/logout.md), like so:
...
Or after [`AnonymousAuthenticationFilter`](authentication/anonymous.md), the last authentication filter in the chain, like so:
...
By adding the filter after the [`AnonymousAuthenticationFilter`](authentication/anonymous.md) we are making sure that the `TenantFilter` is invoked after the authentication filters.
...
For example, calling [ `HttpSecurity#httpBasic`](authentication/passwords/basic.md) adds a `BasicAuthenticationFilter` for you.
...
![exceptiontranslationfilter](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/servlet/architecture/exceptiontranslationfilter.png)

- ![number 1](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/icons/number_1.png) First, the `ExceptionTranslationFilter` invokes `FilterChain.doFilter(request, response)` to invoke the rest of the application.
- ![number 2](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/icons/number_2.png) If the user is not authenticated or it is an `AuthenticationException`, then *Start Authentication*.
...
- ![number 3](https://raw.githubusercontent.com/spring-projects/spring-security/7.1.1/docs/modules/ROOT/assets/images/icons/number_3.png) Otherwise, if it is an `AccessDeniedException`, then *Access Denied*.
...
This means that if another part of the application, ([`AuthorizationFilter`](authorization/authorize-http-requests.md) or method security) throws an `AuthenticationException` or `AccessDeniedException` it is caught and handled here.
...
The `HttpServletRequest` is saved in the [`RequestCache`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/web/savedrequest/RequestCache.html).
...
To do that, you can use the [NullRequestCache](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/web/savedrequest/NullRequestCache.html) implementation.

#### Prevent the Request From Being Saved - Java
...
#### Prevent the Request From Being Saved - Kotlin
...
#### Prevent the Request From Being Saved - Xml
...
The [`RequestCacheAwareFilter`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/web/savedrequest/RequestCacheAwareFilter.html) uses the [`RequestCache`](#requestcache) to replay the original request.
