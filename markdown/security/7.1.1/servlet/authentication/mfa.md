...
source: "ROOT:servlet/authentication/mfa.adoc"
---

# Multi-Factor Authentication

[Multi-Factor Authentication (MFA)](https://cheatsheetseries.owasp.org/cheatsheets/Multifactor_Authentication_Cheat_Sheet.html) requires that a user provide factors in order to authenticate.
...
<a id="_factorgrantedauthority"></a>

## `FactorGrantedAuthority`

At the time of authentication, Spring Security’s authentication mechanisms add a [`FactorGrantedAuthority`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/core/authority/FactorGrantedAuthority.html).
...
[`@EnableMultiFactorAuthentication`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/config/annotation/authorization/EnableMultiFactorAuthentication.html) makes it easy to enable multifactor authentication.
...
1. Every other URL requires the authorities `FACTOR_OTT`, `FACTOR_PASSWORD`
1. Set up the authentication mechanisms that can provide the required factors.
...
<a id="mfa-when-webauthn-registered"></a>
...
You can achieve this by specifying [`EnableMultiFactorAuthentication.when()`](<https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/config/annotation/authorization/EnableMultiFactorAuthentication.html#when()>).
...
It works by publishing a [`Customizer<AdditionalRequiredFactorsBuilder<Object>>`](#mfa-when-custom-conditions) that updates the [`withWhen`](#programmatic-mfa) method with the condition.

> [!NOTE]
> This condition requires both a [`PublicKeyCredentialUserEntityRepository`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/web/webauthn/management/PublicKeyCredentialUserEntityRepository.html) bean and a [`UserCredentialRepository`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/web/webauthn/management/UserCredentialRepository.html) bean to be published in order to determine if the user has registered a WebAuthn credential.

<a id="mfa-when-custom-conditions"></a>
...
<a id="authorization-manager-factory"></a>

## AuthorizationManagerFactory

The `@EnableMultiFactorAuthentication` `authorities` property is just a shortcut for publishing an [`AuthorizationManagerFactory`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/authorization/AuthorizationManagerFactory.html) Bean.
...
The `AuthorizationManagerFactory` Bean below is what is published in the previously discussed [`@EnableMultiFactorAuthentication` example](#emfa).
...
<a id="selective-mfa"></a>

## Selectively Requiring MFA

We have demonstrated how to configure an entire application to require MFA by using [`@EnableMultiFactorAuthentication`s](#emfa) `authorities` property.
...
Fortunately, we can use what we learned in [AuthorizationManagerFactory](#authorization-manager-factory) to solve this in a concise manner.
...
1. Explicitly use `AuthorizationManagerFactory` so that URLs that begin with `/admin/**` require `FACTOR_OTT`, `FACTOR_PASSWORD`, and `ROLE_ADMIN`.
1. Explicitly use `AuthorizationManagerFactory` so that URLs that begin with `/user/settings` require `FACTOR_OTT` and `FACTOR_PASSWORD`
1. Otherwise, the request must be authenticated.
There is no MFA requirement, because the `AuthorizationManagerFactory` is not used.
1. Set up the authentication mechanisms that can provide the required factors.

<a id="valid-duration"></a>
...
1. Next, we define `passwordInHour` as a requirement for a password within an hour
1. We use `passwordIn30m` to require that URLs that begin with `/admin/` should require that a password has been provided in the last 30 minutes and that the user has the `ROLE_ADMIN` authority
1. We use `passwordInHour` to require that URLs that begin with `/user/settings` should require that a password has been provided in the last hour
1. Otherwise, authentication is required, but it does not care if it is a password or how long ago authentication occurred
1. Set up the authentication mechanisms that can provide the required factors.

<a id="all-factors-anyof"></a>
...
[AllRequiredFactorsAuthorizationManager.anyOf](<https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/authorization/AllRequiredFactorsAuthorizationManager.html#anyOf(AllRequiredFactorsAuthorizationManager\...)>) grants access if at least one of the provided combinations of factors is satisfied.
...
1. Require WebAuthn
1. Require both a password and a one-time token
1. Combine the combinations of factors with `anyOf`, granting access if either is satisfied
1. URLs that begin with `/protected/**` require the user to satisfy either combination of factors
1. All other requests require only authentication
1. Set up the authentication mechanisms that can provide the required factors

<a id="programmatic-mfa"></a>
...
This is implemented using [`ConditionalAuthorizationManager`](../authorization/architecture.md#authz-conditional-authorization-manager).
...
1. Only apply the requirement if the username is `admin`. Otherwise, MFA is not required.
1. Return the `AuthorizationManagerFactory` using `.build()`. Since it is published as a Bean, it is used globally.

This should feel very similar to our previous example in [AuthorizationManagerFactory](#authorization-manager-factory).
...
> [!NOTE]
> MFA is enabled by username and not role because that is how we implemented `RequiredAuthoritiesAuthorizationManagerConfiguration`.
...
We’ve demonstrated how we can dynamically determine the authorities for a particular user in [Programmatic MFA](#programmatic-mfa) using `AuthorizationManagerFactories.multiFactor().when`.
However, this is such a common scenario that Spring Security provides built in support using [`RequiredAuthoritiesAuthorizationManager`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/authorization/RequiredAuthoritiesAuthorizationManager.html) and [`RequiredAuthoritiesRepository`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/authorization/RequiredAuthoritiesRepository.html).

Let’s implement the same requirement that we did in [Programmatic MFA](#programmatic-mfa) using the built-in support.
...
1. Create a [`MapRequiredAuthoritiesRepository`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/authorization/MapRequiredAuthoritiesRepository.html) that maps users with the username `admin` to require MFA.
1. Return a `RequiredAuthoritiesAuthorizationManager` that is injected with the `MapRequiredAuthoritiesRepository`.
...
1. Inject the `RequiredAuthoritiesAuthorizationManager` as the [DefaultAuthorization.additionalAuthorization](<https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/authorization/DefaultAuthorizationManagerFactory.html#setAdditionalAuthorization(org.springframework.security.authorization.AuthorizationManager)>).
...
1. Publish `DefaultAuthorizationManagerFactory` as a Bean, so it is used globally
...
For more dynamic use cases that can be determined by the username, a custom implementation of [`RequiredAuthoritiesRepository`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/authorization/RequiredAuthoritiesRepository.html) can be created.
...
For cases that need to determine MFA based upon the `Authentication`, `AuthorizationManagerFactories.multiFactor().when` can be used as demonstrated in [Programmatic MFA](#programmatic-mfa).

<a id="hasallauthorities"></a>
...
1. For every other URL, the following authorities are required `FACTOR_OTT`, `FACTOR_PASSWORD`, `ROLE_USER`.
1. Set up the authentication mechanisms that can provide the required factors.
...
The answer to these questions, as we have already seen, is to use [@EnableMultiFactorAuthentication](#emfa)

<a id="re-authentication"></a>
...
<a id="obtaining-more-authorization"></a>
