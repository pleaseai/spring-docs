...
You also need to add Spring Security’s `TestSecurityContextHolderPostProcessor` to support [Running as a User in Spring MVC Test with Annotations](authentication.md#test-mockmvc-withmockuser).
...
Now that you have configured MockMvc with `springSecurity()`, you can use Security’s [RequestPostProcessors](request-post-processors.md) and other MockMvc test support.
