...
source: "ROOT:servlet/configuration/xml-namespace.adoc"
---
...
You can find more information in the Spring [Reference Documentation](https://docs.spring.io/spring-framework/reference/7.0.9/index.html).
...
You can find out more about the use of the `ldap-server` element in the chapter on [LDAP Authentication](../authentication/passwords/ldap.md#servlet-authentication-ldap).
...
We recommend that you try the [Spring Tool Suite](https://spring.io/tools/sts), as it has special features for working with standard Spring namespaces.
...
<a id="_design_of_the_namespace"></a>
...
- *Web/HTTP Security* is the most complex part.
...
- *AuthenticationProvider* instances provide mechanisms against which the authentication manager authenticates users.
...
The `<intercept-url>` element defines a `pattern`, which is matched against the URLs of incoming requests using Ant path syntax. See the section on [`HttpFirewall`](../exploits/firewall.md#servlet-httpfirewall) for more details on how matches are actually performed.
...
We see later how the interpretation can vary. The interpretation of the comma-separated values in the `access` attribute depends on the which implementation of the [`AccessDecisionManager`](#ns-access-manager) is used.
Since Spring Security 3.0, you can also populate the attribute with an  [EL expression](../authorization/authorize-http-requests.md#authorization-expressions).
...
> You can find more detailed information on the beans that are created in the [namespace appendix](../appendix/namespace/index.md#appendix-namespace).
...
See the section on [in-memory authentication](../authentication/passwords/in-memory.md#servlet-authentication-inmemory) for more details on the file format.
...
| --- | --- | --- |
| DISABLE\_ENCODE\_URL\_FILTER | `DisableEncodeUrlFilter` | `http@disable-url-rewriting` |
| FORCE\_EAGER\_SESSION\_FILTER | `ForceEagerSessionCreationFilter` | `http@create-session="ALWAYS"` |
...
> By default, an `AnonymousAuthenticationFilter` is added and unless you have [session-fixation protection](../authentication/session-management.md#ns-session-fixation) disabled, a `SessionManagementFilter` is also added to the filter chain.
...
Since version 3.0, you can also make use of [expression-based annotations](../authorization/method-security.md#authorizing-with-annotations).
...
You can find out more about these in the chapter on [authorization](../authorization/architecture.md#authz-arch).

<a id="ns-custom-access-mgr"></a>
