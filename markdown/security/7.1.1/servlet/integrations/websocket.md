...
Spring Security 4 added support for securing [Spring’s WebSocket support](https://docs.spring.io/spring-framework/reference/7.0.9/web/websocket.html).
...
1. Any message that has a destination starting with `"/app/"` will be required the user to have the role `ROLE_USER`
1. Any message that starts with `"/user/"` or `"/topic/friends/"` that is of type SUBSCRIBE will require `ROLE_USER`
...
<a id="migrating-spel-expressions"></a>
...
In general, it is common for applications to deny any `MESSAGE` sent to a destination that starts with the [broker prefix](https://docs.spring.io/spring-framework/reference/7.0.9/web/websocket/stomp.html) (`/topic/` or `/queue/`).
...
In general, it is common for applications to deny any `SUBSCRIBE` sent to a message that starts with the [broker prefix](https://docs.spring.io/spring-framework/reference/7.0.9/web/websocket/stomp.html) (`/topic/` or `/queue/`).
...
The Spring Framework reference documentation contains a section titled [“Flow of Messages”](https://docs.spring.io/spring-framework/reference/7.0.9/web/websocket/stomp/message-flow.html) that describes how messages flow through the system.
...
Fortunately, since Spring 4.1.5 Spring’s WebSocket and SockJS support restricts access to the [current domain](https://docs.spring.io/spring-framework/reference/7.0.9/web/websocket/server.html#websocket-server-allowed-origins).
...
[SockJS](https://docs.spring.io/spring-framework/reference/7.0.9/web/websocket/fallback.html) provides fallback transports to support older browsers.
...
Please see {site-url}/5.8/migration/servlet/authorization.html#\_use\_authorizationmanager\_for\_message\_security\[the 5.8 migration guide\] for guidance.
