...
Spring Security provides [`AesGcmBytesEncryptor`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/crypto/encrypt/AesGcmBytesEncryptor.html) and [`AesCbcBytesEncryptor`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/crypto/encrypt/AesCbcBytesEncryptor.html) for constructing symmetric encryptors.
These can be used to encrypt data in raw `byte[]` form.
You can also construct [`TextEncryptor`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/crypto/encrypt/TextEncryptor.html) instances to encrypt text strings.
...
`AesGcmBytesEncryptor` uses 256-bit AES encryption with Galois Counter Mode (GCM), providing [authenticated encryption](https://en.wikipedia.org/wiki/Authenticated_encryption) (AEAD).
...
> [!NOTE]
> Queryable text encryption (encrypting such that the same plaintext always produces the same ciphertext) is no longer recommended, as it relies on a fixed initialization vector and does not provide adequate security.
...
The [`KeyGenerators`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/crypto/keygen/KeyGenerators.html) class provides a number of convenience factory methods for constructing different types of key generators.
By using this class, you can create a [`BytesKeyGenerator`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/crypto/keygen/BytesKeyGenerator.html) to generate `byte[]` keys.
You can also construct a [`StringKeyGenerator`](https://docs.spring.io/spring-security/site/docs/7.1.1/api/org/springframework/security/crypto/keygen/StringKeyGenerator.html) to generate string keys.
