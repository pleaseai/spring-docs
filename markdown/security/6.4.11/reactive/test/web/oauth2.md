...
source: "ROOT:reactive/test/web/oauth2.adoc"
---

<a id="webflux-testing-oauth2"></a>

# Testing OAuth 2.0

When it comes to OAuth 2.0, [the same principles covered earlier still apply](../method.md#test-erms): Ultimately, it depends on what your method under test is expecting to be in the `SecurityContextHolder`.
...
Nothing about it is OAuth2-specific, so you can [use `@WithMockUser`](../method.md#test-erms) and be fine.
...
<a id="webflux-testing-oidc-login"></a>

## Testing OIDC Login

Testing the method shown in the [preceding section](#webflux-testing-oauth2) with `WebTestClient` requires simulating some kind of grant flow with an authorization server.
...
Spring Security makes sure that the `OidcUser` instance is available for[the `@AuthenticationPrincipal` annotation](../../../servlet/integrations/mvc.md#mvc-authentication-principal).
...
This can be handy if your tests [use the `@RegisteredOAuth2AuthorizedClient` annotation](#webflux-testing-oauth2-client)..

<a id="webflux-testing-oidc-login-authorities"></a>
...
<a id="webflux-testing-oidc-login-claims"></a>
...
<a id="webflux-testing-oidc-login-user"></a>
...
<a id="webflux-testing-oauth2-login"></a>

## Testing OAuth 2.0 Login

As with [testing OIDC login](#webflux-testing-oidc-login), testing OAuth 2.0 Login presents a similar challenge: mocking a grant flow.
...
Spring Security does the necessary work to make sure that the `OAuth2User` instance is available for [the `@AuthenticationPrincipal` annotation](../../../servlet/integrations/mvc.md#mvc-authentication-principal).
...
<a id="webflux-testing-oauth2-login-authorities"></a>
...
<a id="webflux-testing-oauth2-login-claims"></a>
...
<a id="webflux-testing-oauth2-login-user"></a>
...
<a id="webflux-testing-oauth2-client"></a>
...
<a id="webflux-testing-oauth2-client-scopes"></a>
...
<a id="webflux-testing-oauth2-client-registration"></a>
...
<a id="_mockjwt_webtestclientconfigurer"></a>
...
You can also specify a complete `Jwt`, for which [`Jwt.Builder`](https://docs.spring.io/spring-security/site/docs/6.4.11/api/org/springframework/security/oauth2/jwt/Jwt.Builder.html) is quite handy:
...
<a id="_authentication_and_webtestclientconfigurer"></a>
...
<a id="webflux-testing-opaque-token"></a>

## Testing Opaque Token Authentication

Similar to [JWTs](#webflux-testing-jwt), opaque tokens require an authorization server in order to verify their validity, which can make testing more difficult.
...
<a id="webflux-testing-opaque-token-authorities"></a>
...
<a id="webflux-testing-opaque-token-attributes"></a>
...
<a id="webflux-testing-opaque-token-principal"></a>
