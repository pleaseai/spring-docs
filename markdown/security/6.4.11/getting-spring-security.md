...
```groovy
ext['spring-security.version']='6.4.11'
```
...
dependencyManagement {
	imports {
		mavenBom 'org.springframework.security:spring-security-bom:6.4.11'
	}
}
```
...
Spring Security builds against Spring Framework 6.2.11 but should generally work with any newer version of Spring Framework 5.x.
Many users are likely to run afoul of the fact that Spring Security’s transitive dependencies resolve Spring Framework 6.2.11, which can cause strange classpath problems.
...
The preceding example ensures that all the transitive dependencies of Spring Security use the Spring 6.2.11 modules.
