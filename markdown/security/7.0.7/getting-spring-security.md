...
<properties>
	<!-- ... -->
	<spring-security.version>7.0.7</spring-security.version>
</properties>
```
...
```groovy
ext['spring-security.version']='7.0.7'
```
...
dependencyManagement {
	imports {
		mavenBom 'org.springframework.security:spring-security-bom:7.0.7'
	}
}
```
...
Spring Security builds against Spring Framework 7.0.9 but should generally work with any newer version of Spring Framework 5.x.
Many users are likely to run afoul of the fact that Spring Security’s transitive dependencies resolve Spring Framework 7.0.9, which can cause strange classpath problems.
...
The preceding example ensures that all the transitive dependencies of Spring Security use the Spring 7.0.9 modules.
