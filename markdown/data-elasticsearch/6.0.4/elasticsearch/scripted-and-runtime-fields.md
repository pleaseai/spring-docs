---
title: "Scripted and runtime fields"
source: "ROOT:elasticsearch/scripted-and-runtime-fields.adoc"
---

<a id="elasticsearch.misc.scripted-and-runtime-fields"></a>
...
1. a convenience constructor to set up the test data.
...
<a id="the-repository-interface"></a>
...
1. when using a `Query`, add the scripted field to the query.
1. when adding a scripted field to a `Query`, an additional source filter is needed to also retrieve the *normal* fields from the document source.
1. get the data where the `Person` entities now have the values set in their `age` property.
1. when using the repository, all that needs to be done is adding the scripted field as method parameter.

<a id="runtime-fields"></a>
...
1. define the runtime field that calculates the age of a person. // see [asciidoctor.org/docs/user-manual/#builtin-attributes](https://asciidoctor.org/docs/user-manual/#builtin-attributes) for builtin attributes.
1. when using `Query`, add the runtime field.
1. when adding a scripted field to a `Query`, an additional field parameter is needed to have the calculated value returned.
1. when adding a scripted field to a `Query`, an additional source filter is needed to also retrieve the *normal* fields from the document source.
1. get the data filtered with the query and where the returned entites have the age property set.
1. when using the repository, all that needs to be done is adding the runtime field as method parameter.
