...
Spring Data JDBC requires [Spring Framework](https://docs.spring.io/spring-framework/reference/6.1) 6.1.4 and above.
...
1. Change the version of Spring in the pom.xml to be
   ...
   ```xml
   <spring.version>6.1.4</spring-framework.version>
   ```
...
> [!TIP]
> Dialects are resolved by [`DialectResolver`](https://docs.spring.io/spring-data/jdbc/docs/3.2.3/api//org/springframework/data/jdbc/repository/config/DialectResolver.html) from a `JdbcOperations` instance, typically by inspecting `Connection.getMetaData()`.
