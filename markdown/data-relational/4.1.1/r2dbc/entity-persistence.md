...
The entry points (`insert()`, `select()`, `update()`, `upsert()`, and others) follow a natural naming schema based on the operation to be run.
...
- `Mono<T>` **upsert** `(T objectToSave)`: Upsert (merge) the object to the default table.
...
<a id="r2dbc.entityoperations.fluent-api.upsert"></a>

## Upserting Data

`R2dbcEntityOperations` provides an `upsert(instance)` operation for cases where you need to persist an entity in a single database round-trip without prior existence checks.
...
Database dialect support is required as well: The feature is available for PostgreSQL, H2, MySQL, MariaDB, SQL Server, Oracle, and HSQLDB.

> [!NOTE]
> Upserts currently do not support optimistic locking.

Upserting data starts by specifying the object to upsert by accepting the `objectToSave`.
Consider the following upsert operation:
...
1. Update `Person` objects and apply mapping based on mapping metadata.
1. Provide the object to be upserted.
