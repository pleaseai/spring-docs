...
Use the store-specific `@EnableJdbcRepositories` annotation on a Java configuration class to define a configuration for repository activation.
...
The `basePackages` and `value` attributes in `@EnableJdbcRepositories` support `${…}` property placeholders which are resolved against the `Environment` as well as Ant-style package patterns such as `"org.example.**"`.

The following example specifies the `app.scan.packages` property placeholder for the implicit `value` attribute in `@EnableJdbcRepositories`.

#### Java

```java
@Configuration
@EnableJdbcRepositories("${app.scan.packages}")    // <1>
