...
Spring Data JDBC requires [Spring Framework](https://docs.spring.io/spring-framework/reference/6.1) 6.1.5 and above.
...
1. Change the version of Spring in the pom.xml to be
   ...
   ```xml
   <spring.version>6.1.5</spring-framework.version>
   ```
...
1. `AbstractJdbcConfiguration` provides various default beans required by Spring Data JDBC
...
> [!TIP]
> Dialects are resolved by [`DialectResolver`](https://docs.spring.io/spring-data/jdbc/docs/3.2.4/api//org/springframework/data/jdbc/repository/config/DialectResolver.html) from a `JdbcOperations` instance, typically by inspecting `Connection.getMetaData()`.
