---
title: "Query Methods"
source: "ROOT:r2dbc/query-methods.adoc"
---

<a id="r2dbc.repositories.queries"></a>
...
1. Unless <4>, the first entity is always emitted even if the query yields more result rows.
1. The `findByLastname` method shows a query for all people with the given last name.
1. A query for a single `Person` entity projecting only `firstname` and `lastname` columns.
...
<a id="r2dbc.repositories.modifying"></a>
...
1. Using `Void` just reports whether the rows were successfully deleted without emitting a result value.
1. Using `Boolean` reports whether at least one row was removed.
...
Alternatively, you can add custom modifying behavior by using the facilities described in [Custom Implementations for Spring Data Repositories](../repositories/custom-implementations.md).

<a id="r2dbc.query-methods.at-query"></a>
...
<a id="r2dbc.repositories.queries.spel"></a>
