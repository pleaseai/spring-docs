...
Spring Data JDBC requires [Spring Framework](https://docs.spring.io/spring-framework/reference/7.0) 7.0.6 and above.
...
Thus, if you want to inspect what SQL statements are run, activate logging for Spring’s [`NamedParameterJdbcTemplate`](https://docs.spring.io/spring-framework/reference/7.0/data-access/jdbc/core.html#jdbc-NamedParameterJdbcTemplate) or [MyBatis](https://www.mybatis.org/mybatis-3/logging.html).
...
1. [`AbstractJdbcConfiguration`](https://docs.spring.io/spring-data/relational/docs/4.0.4/api/org/springframework/data/jdbc/repository/config/AbstractJdbcConfiguration.html) provides various default beans required by Spring Data JDBC.
...
Extending [`AbstractJdbcConfiguration`](https://docs.spring.io/spring-data/relational/docs/4.0.4/api/org/springframework/data/jdbc/repository/config/AbstractJdbcConfiguration.html) ensures various beans get registered.
...
> [!TIP]
> Dialects are resolved by [`DialectResolver`](https://docs.spring.io/spring-data/relational/docs/4.0.4/api/org/springframework/data/jdbc/core/dialect/DialectResolver.html) from a `JdbcOperations` instance, typically by inspecting `Connection.getMetaData()`.
>
> You can let Spring auto-discover your [`JdbcDialect`](https://docs.spring.io/spring-data/relational/docs/4.0.4/api/org/springframework/data/jdbc/core/dialect/JdbcDialect.html) by registering a class that implements `org.springframework.data.jdbc.core.dialect.DialectResolver$JdbcDialectProvider` through `META-INF/spring.factories`.
> `DialectResolver` discovers dialect provider implementations from the class path using Spring’s `SpringFactoriesLoader`.
> To do so:
>
> 1. Implement your own `JdbcDialect`.
> 1. Implement a `JdbcDialectProvider` returning the `JdbcDialect`.
