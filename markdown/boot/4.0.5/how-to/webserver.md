...
To disable this behavior configure the [`WebApplicationType`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/WebApplicationType.html) in your `application.properties`, as shown in the following example:
...
You can access the port the server is running on from log output or from the [`WebServerApplicationContext`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/server/context/WebServerApplicationContext.html) through its [`WebServer`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/server/WebServer.html).
...
Tests that use `@SpringBootTest(webEnvironment=WebEnvironment.RANDOM_PORT)` can also inject the actual port into a field by using the [`@LocalServerPort`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/test/web/server/LocalServerPort.html) annotation, as shown in the following example:
...
> [!NOTE]
> [`@LocalServerPort`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/test/web/server/LocalServerPort.html) is a meta-annotation for `@Value("${local.server.port}")`.
...
Tests that start a [`WebServer`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/server/WebServer.html) such as  `@SpringBootTest(webEnvironment=WebEnvironment.RANDOM_PORT)` can get information about
the running server using [`LocalTestWebServer`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/test/http/server/LocalTestWebServer.html), as shown in the following example:
...
[`LocalTestWebServer`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/test/http/server/LocalTestWebServer.html) provides access to a suitable [`UriBuilderFactory`](https://docs.spring.io/spring-framework/docs/7.0.x/javadoc-api/org/springframework/web/util/UriBuilderFactory.html).
...
See [`Ssl`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/server/Ssl.html) for details of all of the supported properties.
...
However, if a configuration key does not exist for your use case, you should then look at [`WebServerFactoryCustomizer`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/server/WebServerFactoryCustomizer.html).
...
> Auto-configured [`WebServerFactoryCustomizer`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/server/WebServerFactoryCustomizer.html) beans have an order of `0` and will be processed before any user-defined customizers, unless it has an explicit order that states otherwise.

Once you have got access to a [`WebServerFactory`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/server/WebServerFactory.html) using the customizer, you can use it to configure specific parts, like connectors, server resources, or the server itself - all using server-specific APIs.
...
| Jetty | [`JettyServletWebServerFactory`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/jetty/servlet/JettyServletWebServerFactory.html) | [`JettyReactiveWebServerFactory`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/jetty/reactive/JettyReactiveWebServerFactory.html) |
| Reactor | N/A | [`NettyReactiveWebServerFactory`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/reactor/netty/NettyReactiveWebServerFactory.html) |

As a last resort, you can also declare your own [`WebServerFactory`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/server/WebServerFactory.html) bean, which will override the one provided by Spring Boot.
...
In the case of filters and servlets, you can also add mappings and init parameters by adding a [`FilterRegistrationBean`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/servlet/FilterRegistrationBean.html) or a [`ServletRegistrationBean`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/servlet/ServletRegistrationBean.html) instead of or in addition to the underlying component.
You can also use [`@ServletRegistration`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/servlet/ServletRegistration.html) and [`@FilterRegistration`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/servlet/FilterRegistration.html) as an annotation-based alternative to [`ServletRegistrationBean`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/servlet/ServletRegistrationBean.html) and [`FilterRegistrationBean`](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/web/servlet/FilterRegistrationBean.html).
...
> [!NOTE]
> If your application runs [in a supported Cloud Platform](https://docs.spring.io/spring-boot/4.0.5/api/java/org/springframework/boot/cloud/CloudPlatform.html#enum-constant-summary), the `server.forward-headers-strategy` property defaults to `NATIVE`.
