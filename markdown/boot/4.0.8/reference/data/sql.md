...
[Spring Data](https://spring.io/projects/spring-data) provides an additional level of functionality: creating [`Repository`](https://docs.spring.io/spring-data/commons/docs/4.0.x/api/org/springframework/data/repository/Repository.html) implementations directly from interfaces and using conventions to generate queries from your method names.
...
See [`DataSourceProperties`](https://docs.spring.io/spring-boot/4.0.8/api/java/org/springframework/boot/jdbc/autoconfigure/DataSourceProperties.html) API documentation for more of the supported options.
...
Additional connection pools can always be configured manually, using [`DataSourceBuilder`](https://docs.spring.io/spring-boot/4.0.8/api/java/org/springframework/boot/jdbc/DataSourceBuilder.html).
...
The following connection pools are supported by [`DataSourceBuilder`](https://docs.spring.io/spring-boot/4.0.8/api/java/org/springframework/boot/jdbc/DataSourceBuilder.html):
...
> [!TIP]
> You can customize entity scanning locations by using the [`@EntityScan`](https://docs.spring.io/spring-boot/4.0.8/api/java/org/springframework/boot/persistence/autoconfigure/EntityScan.html) annotation.
...
For more complex queries, you can annotate your method with Spring Data’s [`Query`](https://docs.spring.io/spring-data/jpa/docs/4.0.x/api/org/springframework/data/jpa/repository/Query.html) annotation.

Spring Data repositories usually extend from the [`Repository`](https://docs.spring.io/spring-data/commons/docs/4.0.x/api/org/springframework/data/repository/Repository.html) or [`CrudRepository`](https://docs.spring.io/spring-data/commons/docs/4.0.x/api/org/springframework/data/repository/CrudRepository.html) interfaces.
...
> [!TIP]
> You can customize the locations to look for repositories using [`@EnableJpaRepositories`](https://docs.spring.io/spring-data/jpa/docs/4.0.x/api/org/springframework/data/jpa/repository/config/EnableJpaRepositories.html).
...
> You can use [`SmartInitializingSingleton`](https://docs.spring.io/spring-framework/docs/7.0.x/javadoc-api/org/springframework/beans/factory/SmartInitializingSingleton.html) to invoke any initialization that requires the JPA infrastructure.
> For JPA components (such as converters) that are created as Spring beans, use [`ObjectProvider`](https://docs.spring.io/spring-framework/docs/7.0.x/javadoc-api/org/springframework/beans/factory/ObjectProvider.html) to delay the resolution of dependencies, if any.

> [!TIP]
> We have barely scratched the surface of Spring Data JPA.
> For complete details, see the [Spring Data JPA reference documentation](https://docs.spring.io/spring-data/jpa/reference/4.0).
...
To use Spring Data Envers, make sure your repository extends from [`RevisionRepository`](https://docs.spring.io/spring-data/commons/docs/4.0.x/api/org/springframework/data/repository/history/RevisionRepository.html) as shown in the following example:
...
> [!NOTE]
> For more details, check the [Spring Data Envers reference documentation](https://docs.spring.io/spring-data/jpa/reference/4.0/envers.html).
...
Spring Data includes repository support for JDBC and will automatically generate SQL for the methods on [`CrudRepository`](https://docs.spring.io/spring-data/commons/docs/4.0.x/api/org/springframework/data/repository/CrudRepository.html).
For more advanced queries, a [`@Query`](https://docs.spring.io/spring-data/jdbc/docs/4.0.x/api/org/springframework/data/jdbc/repository/query/Query.html) annotation is provided.
...
If necessary, you can take control of Spring Data JDBC’s configuration by adding the [`@EnableJdbcRepositories`](https://docs.spring.io/spring-data/jdbc/docs/4.0.x/api/org/springframework/data/jdbc/repository/config/EnableJdbcRepositories.html) annotation or an [`AbstractJdbcConfiguration`](https://docs.spring.io/spring-data/jdbc/docs/4.0.x/api/org/springframework/data/jdbc/repository/config/AbstractJdbcConfiguration.html) subclass to your application.

If you’re using Spring Data JDBC with [ahead-of-time processing](../packaging/aot.md) (targeting either the JVM or a native image), some additional configuration is recommended.
...
> [!TIP]
> For complete details of Spring Data JDBC, see the [reference documentation](https://docs.spring.io/spring-data/relational/reference/4.0).
...
More information on [CSRF](https://docs.spring.io/spring-security/reference/7.0/features/exploits/csrf.html) and the header [X-Frame-Options](https://docs.spring.io/spring-security/reference/7.0/features/exploits/headers.html#headers-frame-options) can be found in the Spring Security Reference Guide.

In simple setups, a [`SecurityFilterChain`](https://docs.spring.io/spring-security/site/docs/7.0.x/api/org/springframework/security/web/SecurityFilterChain.html) like the following can be used:
...
You can follow the instructions in the [jOOQ user manual](https://www.jooq.org/doc/3.19.37/manual-single-page/#jooq-in-7-steps-step3).
...
The fluent API offered by jOOQ is initiated through the [`DSLContext`](https://www.jooq.org/javadoc/3.19.37/org/jooq/DSLContext.html) interface.
Spring Boot auto-configures a [`DSLContext`](https://www.jooq.org/javadoc/3.19.37/org/jooq/DSLContext.html) as a Spring Bean and connects it to your application [`DataSource`](https://docs.oracle.com/en/java/javase/17/docs/api/java.sql/javax/sql/DataSource.html).
To use the [`DSLContext`](https://www.jooq.org/javadoc/3.19.37/org/jooq/DSLContext.html), you can inject it, as shown in the following example:
...
> [!TIP]
> The jOOQ manual tends to use a variable named `create` to hold the [`DSLContext`](https://www.jooq.org/javadoc/3.19.37/org/jooq/DSLContext.html).

You can then use the [`DSLContext`](https://www.jooq.org/javadoc/3.19.37/org/jooq/DSLContext.html) to construct your queries, as shown in the following example:
...
More advanced customizations can be achieved by defining your own [`DefaultConfigurationCustomizer`](https://docs.spring.io/spring-boot/4.0.8/api/java/org/springframework/boot/jooq/autoconfigure/DefaultConfigurationCustomizer.html) bean that will be invoked prior to creating the [`Configuration`](https://www.jooq.org/javadoc/3.19.37/org/jooq/Configuration.html) [`@Bean`](https://docs.spring.io/spring-framework/docs/7.0.x/javadoc-api/org/springframework/context/annotation/Bean.html).
This takes precedence to anything that is applied by the auto-configuration.

You can also create your own [`Configuration`](https://www.jooq.org/javadoc/3.19.37/org/jooq/Configuration.html) [`@Bean`](https://docs.spring.io/spring-framework/docs/7.0.x/javadoc-api/org/springframework/context/annotation/Bean.html) if you want to take complete control of the jOOQ configuration.
...
For more complex queries, you can annotate your method with Spring Data’s [`@Query`](https://docs.spring.io/spring-data/r2dbc/docs/4.0.x/api/org/springframework/data/r2dbc/repository/Query.html) annotation.
...
> [!TIP]
> We have barely scratched the surface of Spring Data R2DBC. For complete details, see the [Spring Data R2DBC reference documentation](https://docs.spring.io/spring-data/relational/reference/4.0).
