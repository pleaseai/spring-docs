...
<a id="howto.deployment.cloud.security"></a>

## Security considerations

Support for [forwarded headers](../webserver.md#howto.webserver.use-behind-a-proxy-server) should only be enabled when the application receives traffic from a trusted HTTP proxy, and is only directly accessible from a trusted network.
...
See the [`CloudFoundryVcapEnvironmentPostProcessor`](https://docs.spring.io/spring-boot/4.0.8/api/java/org/springframework/boot/cloud/CloudFoundryVcapEnvironmentPostProcessor.html) API documentation for complete details.
...
Spring Boot makes `-D` arguments available as properties accessible from a Spring [`Environment`](https://docs.spring.io/spring-framework/docs/7.0.x/javadoc-api/org/springframework/core/env/Environment.html) instance.
...
<a id="howto.deployment.cloud.aws.ecs"></a>
...
> [!NOTE]
> Spring Boot applications can be packaged in Docker containers using techniques described in [reference:packaging/container-images/index.adoc](../../reference/packaging/container-images/index.md).
...
> [!TIP]
> Spring Boot auto-detects AWS ECS deployment environments by checking the environment for the `AWS_EXECUTION_ENV` variable.
> You can override this detection with the `spring.main.cloud-platform` configuration property.
...
> [!NOTE]
> Creating deployable war for a Spring Boot application is described in [deployment/traditional-deployment.adoc](traditional-deployment.md).
