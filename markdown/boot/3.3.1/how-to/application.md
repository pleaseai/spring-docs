...
[`FailureAnalyzer`](https://docs.spring.io/spring-boot/3.3.1/api/java/org/springframework/boot/diagnostics/FailureAnalyzer.html) is a great way to intercept an exception on startup and turn it into a human-readable message, wrapped in a [`FailureAnalysis`](https://docs.spring.io/spring-boot/3.3.1/api/java/org/springframework/boot/diagnostics/FailureAnalysis.html).
...
If you use the `spring-boot-actuator` (see [the Actuator chapter](actuator.md)), there is also a `conditions` endpoint that renders the report in JSON.
...
- Look for classes that are `@ConfigurationProperties` (such as [`ServerProperties`](https://docs.spring.io/spring-boot/3.3.1/api/java/org/springframework/boot/autoconfigure/web/ServerProperties.html)) and read from there the available external configuration options.
...
See “[Application Events and Listeners](../reference/features/spring-application.md#features.spring-application.application-events-and-listeners)” in the ‘Spring Boot features’ section for a complete list.
...
See “[Fluent Builder API](../reference/features/spring-application.md#features.spring-application.fluent-builder-api)” in the ‘Spring Boot features’ section for more information.
