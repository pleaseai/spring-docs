...
If you need the `MockK` equivalent of the Mockito specific [`@MockBean` and [`@SpyBean`](https://docs.spring.io/spring-boot/3.3.12/api/java/org/springframework/boot/test/mock/mockito/SpyBean.html) annotations](../testing/spring-boot-applications.md#testing.spring-boot-applications.mocking-beans), you can use [SpringMockK](https://github.com/Ninja-Squad/springmockk) which provides similar `@MockkBean` and `@SpykBean` annotations.
