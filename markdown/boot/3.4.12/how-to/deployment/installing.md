...
source: "how-to:deployment/installing.adoc"
---

<a id="howto.deployment.installing"></a>
...
<a id="howto.deployment.installing.system-d"></a>
...
<a id="howto.deployment.installing.init-d"></a>
...
<a id="howto.deployment.installing.init-d.securing"></a>

### Securing an init.d Service

> [!NOTE]
> The following is a set of guidelines on how to secure a Spring Boot application that runs as an init.d service.
...
If root is used to control the application’s service and you [use a `.conf` file](#howto.deployment.installing.init-d.script-customization.when-running.conf-file) to customize its startup, the `.conf` file is read and evaluated by the root user.
...
<a id="howto.deployment.installing.init-d.script-customization"></a>
...
<a id="howto.deployment.installing.init-d.script-customization.when-written"></a>
...
To customize written elements, use the `embeddedLaunchScriptProperties` option of the Spring Boot Maven plugin or the [`properties` property of the Spring Boot Gradle plugin’s `launchScript`](https://docs.spring.io/spring-boot/3.4.12/gradle-plugin/packaging.html#packaging-executable.configuring.launch-script).
...
<a id="howto.deployment.installing.init-d.script-customization.when-running"></a>

#### Customizing a Script When It Runs

For items of the script that need to be customized *after* the jar has been written, you can use environment variables or a [config file](#howto.deployment.installing.init-d.script-customization.when-running.conf-file).
...
| `USE_START_STOP_DAEMON` | Whether the `start-stop-daemon` command, when it is available, should be used to control the process. Defaults to `true`. |
...
> See the [service unit configuration man page](https://www.freedesktop.org/software/systemd/man/systemd.service.html) for more details.

<a id="howto.deployment.installing.init-d.script-customization.when-running.conf-file"></a>
...
To learn about securing this file appropriately, see [the guidelines for securing an init.d service](#howto.deployment.installing.init-d.securing).

<a id="howto.deployment.installing.windows-services"></a>
