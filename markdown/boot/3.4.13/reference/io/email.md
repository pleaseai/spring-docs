...
> [!TIP]
> See the [reference documentation](https://docs.spring.io/spring-framework/reference/6.2/integration/email.html) for a detailed explanation of how you can use [`JavaMailSender`](https://docs.spring.io/spring-framework/docs/6.2.x/javadoc-api/org/springframework/mail/javamail/JavaMailSender.html).

If `spring.mail.host` and the relevant libraries (as defined by `spring-boot-starter-mail`) are available, a default [`JavaMailSender`](https://docs.spring.io/spring-framework/docs/6.2.x/javadoc-api/org/springframework/mail/javamail/JavaMailSender.html) is created if none exists.
The sender can be further customized by configuration items from the `spring.mail` namespace.
See [`MailProperties`](https://docs.spring.io/spring-boot/3.4.13/api/java/org/springframework/boot/autoconfigure/mail/MailProperties.html) for more details.
