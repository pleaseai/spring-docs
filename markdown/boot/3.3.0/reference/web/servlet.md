...
The [Spring Web MVC framework](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc.html) (often referred to as “Spring MVC”) is a rich “model view controller” web framework.
...
Spring MVC is part of the core Spring Framework, and detailed information is available in the [reference documentation](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc.html).
...
- Support for `HttpMessageConverters` (covered [later in this document](#web.servlet.spring-mvc.message-converters)).
- Automatic registration of `MessageCodesResolver` (covered [later in this document](#web.servlet.spring-mvc.message-codes)).
- Static `index.html` support.
- Automatic use of a `ConfigurableWebBindingInitializer` bean (covered [later in this document](#web.servlet.spring-mvc.binding-initializer)).

If you want to keep those Spring Boot MVC customizations and make more [MVC customizations](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc.html) (interceptors, formatters, view controllers, and other features), you can add your own `@Configuration` class of type `WebMvcConfigurer` but **without** `@EnableWebMvc`.
...
If you set the `spring.mvc.message-codes-resolver-format` property `PREFIX_ERROR_CODE` or `POSTFIX_ERROR_CODE`, Spring Boot creates one for you (see the enumeration in [`DefaultMessageCodesResolver.Format`](https://docs.spring.io/spring-framework/docs/6.1.8/javadoc-api/org/springframework/validation/DefaultMessageCodesResolver.Format.html)).
...
> Other template engines are currently not automatically supported but can be with custom template macros/helpers and the use of the [`ResourceUrlProvider`](https://docs.spring.io/spring-framework/docs/6.1.8/javadoc-api/org/springframework/web/servlet/resource/ResourceUrlProvider.html).
...
See [`WebProperties.Resources`](https://docs.spring.io/spring-boot/3.3.0/api/java/org/springframework/boot/autoconfigure/web/WebProperties.Resources.html) for more supported options.

> [!TIP]
> This feature has been thoroughly described in a dedicated [blog post](https://spring.io/blog/2014/07/24/spring-framework-4-1-handling-static-web-resources) and in Spring Framework’s [reference documentation](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc/mvc-config/static-resources.html).
...
This is considered as a [best practice for Spring MVC applications](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc/mvc-controller/ann-requestmapping.html#mvc-ann-requestmapping-suffix-pattern-match).
...
`PathPatternParser` restricts usage of [some path pattern variants](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc/mvc-controller/ann-requestmapping.html#mvc-ann-requestmapping-uri-templates).
...
As of Spring Framework 6.0, [RFC 7807 Problem Details](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc/mvc-ann-rest-exceptions.html) is supported.
...
Applications can ensure that such exceptions are recorded with the observations by [setting the handled exception on the observation context](https://docs.spring.io/spring-framework/reference/6.1.8/integration/observability.html#observability.http-server.servlet).
...
You can also use regular Spring MVC features such as [`@ExceptionHandler` methods](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc/mvc-servlet/exceptionhandlers.html) and [`@ControllerAdvice`](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc/mvc-controller/ann-advice.html).
...
As of version 4.2, Spring MVC [supports CORS](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc-cors.html).
Using [controller method CORS configuration](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc-cors.html#mvc-cors-controller) with [`@CrossOrigin`](https://docs.spring.io/spring-framework/docs/6.1.8/javadoc-api/org/springframework/web/bind/annotation/CrossOrigin.html) annotations in your Spring Boot application does not require any specific configuration.
[Global CORS configuration](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc-cors.html#mvc-cors-global) can be defined by registering a `WebMvcConfigurer` bean with a customized `addCorsMappings(CorsRegistry)` method, as shown in the following example:
...
> For example, it cannot scan for endpoints in a package found in a [fully executable jar file](../deployment/installing.md) or in `WEB-INF/classes` when running an executable war file.
...
> [!TIP]
> To see the order of every `Filter` in your application, enable debug level logging for the `web` [logging group](../features/logging.md#features.logging.log-groups) (`logging.level.web=debug`).
...
> If you need to register a `Filter` that interacts with other beans, consider using a [`DelegatingFilterProxyRegistrationBean`](https://docs.spring.io/spring-boot/3.3.0/api/java/org/springframework/boot/web/servlet/DelegatingFilterProxyRegistrationBean.html) instead.
...
> [!TIP]
> See the [`ServerProperties`](https://docs.spring.io/spring-boot/3.3.0/api/java/org/springframework/boot/autoconfigure/web/ServerProperties.html) class for a complete list.
...
See the [source code documentation](https://docs.spring.io/spring-boot/3.3.0/api/java/org/springframework/boot/web/servlet/server/ConfigurableServletWebServerFactory.html) for details.
