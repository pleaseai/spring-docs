...
See [`Ssl`](https://docs.spring.io/spring-boot/3.3.0/api/java/org/springframework/boot/web/server/Ssl.html) for details of all of the supported properties.
...
See “[Discover Built-in Options for External Properties](properties-and-configuration.md#howto.properties-and-configuration.discover-build-in-options-for-external-properties)”).
...
See the list of [Common Application Properties](../appendix/application-properties/index.md).

The previous sections covered already many common use cases, such as compression, SSL or HTTP/2.
However, if a configuration key does not exist for your use case, you should then look at [`WebServerFactoryCustomizer`](https://docs.spring.io/spring-boot/3.3.0/api/java/org/springframework/boot/web/server/WebServerFactoryCustomizer.html).
...
Like any other Spring bean, you can define the order of servlet filter beans; please make sure to check the “[Registering Servlets, Filters, and Listeners as Spring Beans](../reference/web/servlet.md#web.servlet.embedded-container.servlets-filters-listeners.beans)” section.
...
If this is not enough, Spring Framework provides a [ForwardedHeaderFilter](https://docs.spring.io/spring-framework/reference/6.1.8/web/webmvc/filters.html#filters-forwarded-headers) for the servlet stack and a [ForwardedHeaderTransformer](https://docs.spring.io/spring-framework/reference/6.1.8/web/webflux/reactive-spring.html#webflux-forwarded-headers) for the reactive stack.
...
> [!NOTE]
> If your application runs [in a supported Cloud Platform](https://docs.spring.io/spring-boot/3.3.0/api/java/org/springframework/boot/cloud/CloudPlatform.html#enum-constant-summary), the `server.forward-headers-strategy` property defaults to `NATIVE`.
