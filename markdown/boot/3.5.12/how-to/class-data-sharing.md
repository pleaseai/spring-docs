...
To enable CDS optimization in a generated Docker image, the buildpack environment variable `BP_JVM_CDS_ENABLED` should be set to `true` when building the image as described in the [Maven plugin](https://docs.spring.io/spring-boot/3.5.12/maven-plugin/build-image.html#build-image.examples.builder-configuration) and [Gradle plugin](https://docs.spring.io/spring-boot/3.5.12/gradle-plugin/packaging-oci-image.html#build-image.examples.builder-configuration) documentation.
