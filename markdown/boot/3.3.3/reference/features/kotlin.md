...
See the [Spring Framework Kotlin support documentation](https://docs.spring.io/spring-framework/reference/6.1.12/languages/kotlin.html) for more information.
...
`@ConfigurationProperties` when used in combination with [constructor binding](external-config.md#features.external-config.typesafe-configuration-properties.constructor-binding) supports classes with immutable `val` properties as shown in the following example:
...
If you need the `MockK` equivalent of the Mockito specific [`@MockBean` and `@SpyBean` annotations](../testing/spring-boot-applications.md#testing.spring-boot-applications.mocking-beans), you can use [SpringMockK](https://github.com/Ninja-Squad/springmockk) which provides similar `@MockkBean` and `@SpykBean` annotations.
