...
Spring Boot tries to auto-configure JMS by looking for a `ConnectionFactory` at the JNDI path (`java:/JmsXA` or `java:/XAConnectionFactory`), and you can use the [`spring.datasource.jndi-name` property](../data/sql.md#data.sql.datasource.jndi) to configure your `DataSource`.
...
The [`XAConnectionFactoryWrapper`](https://docs.spring.io/spring-boot/3.3.2/api/java/org/springframework/boot/jms/XAConnectionFactoryWrapper.html) and [`XADataSourceWrapper`](https://docs.spring.io/spring-boot/3.3.2/api/java/org/springframework/boot/jdbc/XADataSourceWrapper.html) interfaces can be used to support embedded transaction managers.
