...
[`FailureAnalyzer`](https://docs.spring.io/spring-boot/3.3.2/api/java/org/springframework/boot/diagnostics/FailureAnalyzer.html) is a great way to intercept an exception on startup and turn it into a human-readable message, wrapped in a [`FailureAnalysis`](https://docs.spring.io/spring-boot/3.3.2/api/java/org/springframework/boot/diagnostics/FailureAnalysis.html).
...
- Look for classes that are `@ConfigurationProperties` (such as [`ServerProperties`](https://docs.spring.io/spring-boot/3.3.2/api/java/org/springframework/boot/autoconfigure/web/ServerProperties.html)) and read from there the available external configuration options.
