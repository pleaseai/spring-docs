...
<a id="upgrading-to-2-0-1"></a>

## Upgrading to 2.0.1

<a id="_redis_chat_memory_repository_auto_configuration_module_renaming"></a>
...
> [!NOTE]
> Both the deprecated module and the legacy configuration prefix will be removed in a future release.

<a id="_mistral_ai_moderation_categories_restructured"></a>
...
<a id="_mistral_ai_chat_model_enumeration_updated"></a>

### Mistral AI Chat Model Enumeration Updated

`MistralAiApi.ChatModel` has been updated to track Mistral AI’s [current model lineup](https://docs.mistral.ai/models/overview#legacy-models). Several constants pointed to models that Mistral AI has since retired.
...
<a id="_mcp_tool_exception_handling"></a>

### MCP Tool Exception Handling

<a id="_changed_mcptool_exception_handling_now_mirrors_tool"></a>
...
<a id="_deprecated_toolcallexceptionclass_constructor_parameter"></a>
...
Most applications won’t notice, since these defaults are generous. If a legitimate workload exceeds them, raise or disable the limit via `ToolCallingManager.builder().maxCallsPerTool(…​)` / `.unlimitedCallsPerTool()` (or the `spring.ai.tools.limits.*` properties). See [Tool Call Limits](api/tools.md#tool-call-limits).

<a id="_changed_toolcallingadvisor_usage_now_accumulated_across_the_loop"></a>
...
<a id="_changed_tool_calling_strict_mode_now_defaults_to_false"></a>
...
<a id="_changed_openaiaudiospeechmodel_streaming_now_emits_real_audio_chunks"></a>
...
Concatenate every emitted chunk instead of taking the first element. See [Bridging a Streaming Response](api/audio/speech.md#streaming-consume-inputstream) for a complete example.

<a id="_transcriptionmodel_now_extends_streamingtranscriptionmodel"></a>
...
<a id="_media_builder_typed_data_overloads_replace_dataobject"></a>
...
<a id="_tool_resolution_fallback_disabled_by_default"></a>
...
See [Tool Resolution](api/tools.md#tool-resolution) for details.
...
For a complete example including the streaming path, see [User-Controlled Tool Execution — With ChatClient](api/tools.md#user-controlled-tool-execution).

<a id="_removed_internal_tool_execution_loop_in_chatmodel"></a>
...
When you genuinely need `ChatModel` directly (custom orchestrator, infrastructure code), drive the loop manually with `ToolCallingManager`. See [ChatModel Tool Calling: Driving the Loop Manually](api/tools/chatmodel-tool-calling.md#driving-the-loop-manually).

<a id="_removed_springbeantoolcallbackresolver_and_toolnames"></a>
...
<a id="_renamed_toolcalladvisor_toolcallingadvisor"></a>
...
<a id="_renamed_functioncallback_api_toolcallback_api"></a>
...
For new code, prefer the declarative `@Tool` annotation — see [Defining Tools](api/tools.md#defining-tools).
...
![Spring AI Dependencies](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/spring-ai-dependencies.png)
...
made available for runtime dynamic resolution (see [Tool Resolution](api/tools.md#tool-resolution)), but never
...
when explicitly requested, you can use one of the strategies described in [Tool Resolution](api/tools.md#tool-resolution).
