...
> See [Securing the MCP Server](#securing-the-mcp-server) for details.
>
...
| --- | --- | --- |
| Server Type | Dependency | Property |
| [SSE WebMVC](<mcp-stdio-sse-server-boot-starter-docs.md#%3Cem%3Esse_webmvc_server>) \_(deprecated since 2.0.0, use STREAMABLE instead) | `spring-ai-starter-mcp-server-webmvc` | `spring.ai.mcp.server.protocol=SSE` |
...
| --- | --- | --- |
| Server Type | Dependency | Property |
| [SSE WebFlux](<mcp-stdio-sse-server-boot-starter-docs.md#%3Cem%3Esse_webflux_server>) \_(deprecated since 2.0.0, use STREAMABLE instead) | `spring-ai-starter-mcp-server-webflux` | `spring.ai.mcp.server.protocol=SSE` |
...
<a id="securing-the-mcp-server"></a>
...
Examples of security libraries are Spring Security and [MCP Security](https://github.com/spring-ai-community/mcp-security)

> [!IMPORTANT]
> With the default configuration the MCP endpoint (`POST /mcp` by default) accepts all requests, allowing any network-accessible client to enumerate and invoke your registered tools, resources, and prompts without credentials.
...
<a id="_adding_data_to_mcptransportcontext"></a>
