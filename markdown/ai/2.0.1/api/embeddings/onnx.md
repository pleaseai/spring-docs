...
source: "ROOT:api/embeddings/onnx.adoc"
---
...
For example, add this to your *application.properties* file to configure the client with the [intfloat/e5-small-v2](https://huggingface.co/intfloat/e5-small-v2) text embedding model:
...
| spring.ai.embedding.transformer.onnx.model-output-name | The ONNX model’s output node name, which we’ll use for embedding calculation. | last\_hidden\_state |
| spring.ai.embedding.transformer.onnx.gpu-device-id | The GPU device ID to execute on. Only applicable if >= 0. Ignored otherwise.(Requires additional onnxruntime\_gpu dependency) | -1 |
...
<a id="_errors_and_special_cases"></a>
...
> that means that you are using the GPU parameters `spring.ai.embedding.transformer.onnx.gpu-device-id` , but the onnxruntime\_gpu dependency are missing.
>
> ```
> <dependency>
...
> ```
>
> Please select the appropriate onnxruntime\_gpu version based on the CUDA version([ONNX Java Runtime](https://onnxruntime.ai/docs/get-started/with-java.html)).
