...
source: "ROOT:api/chat/bedrock-converse.adoc"
---
...
> While the existing [InvokeModel API](../bedrock-chat.md) supports conversation applications, we strongly recommend adopting the Converse API for all Chat conversation models.
>
...
AWS Bedrock’s [prompt caching feature](https://docs.aws.amazon.com/bedrock/latest/userguide/prompt-caching.html) allows you to cache frequently used prompts to reduce costs and improve response times for repeated interactions.
...
> [!NOTE]
> This strategy uses 2 cache breakpoints (one for tools, one for system).
...
<a id="_native_usage_object_recommended_for_observability"></a>
...
<a id="_metadata_map_backward_compatible"></a>
...
<a id="_multi_tenant_saas_application"></a>
...
1. **Meet Token Requirements**: Focus on caching content that meets the minimum token requirements (1024+ tokens for most models).
1. **Reuse Identical Content**: Caching works best with exact matches of prompt content.
Even small changes will require a new cache entry.
1. **Monitor Token Usage**: Track cache effectiveness using the metadata metrics:
   ...
   ```
1. **Strategic Cache Placement**: The implementation automatically places cache breakpoints at optimal locations based on your chosen strategy, ensuring compliance with AWS Bedrock’s 4-breakpoint limit.
1. **Cache Lifetime**: AWS Bedrock caches have a default 5-minute TTL (Time To Live), but they also support a 1-hour TTL duration.
...
1. **Model Compatibility**: Be aware of model-specific limitations:
...
1. **Tool Stability**: When using `TOOLS_ONLY`, `SYSTEM_AND_TOOLS`, or `CONVERSATION_HISTORY` strategies, ensure tools remain stable.
Changing tool definitions will invalidate all downstream cache breakpoints due to cascade invalidation.

<a id="_cache_invalidation_and_cascade_behavior"></a>
...
1. **Provider Portability**: Cache configuration is done through `BedrockChatOptions` rather than individual messages, preserving compatibility when switching between different AI providers.
...
1. **UNION Type Pattern**: AWS SDK uses UNION types where cache points are added as separate blocks rather than properties.
This is different from direct API approaches but ensures type safety and API compliance.
1. **Incremental Caching**: The `CONVERSATION_HISTORY` strategy places cache breakpoints on the last user message, enabling incremental caching where each conversation turn builds on the previous cached prefix.

<a id="_cost_considerations"></a>
...
For most applications, use `ChatClient` with the auto-registered `ToolCallingAdvisor` — see [Tool Calling](../tools.md). For low-level control where you drive the loop yourself (for example, when combining tool calling with prompt caching), see [ChatModel Tool Calling](../tools/chatmodel-tool-calling.md).

<a id="_reasoning_with_tool_calls"></a>
...
This feature is available for [supported models](https://docs.aws.amazon.com/bedrock/latest/userguide/conversation-inference-supported-models-features.html) including Anthropic Claude and Amazon Nova.

<a id="_using_chatclient_with_native_structured_output"></a>
...
<a id="_using_outputschema_directly"></a>
...
For more information, see the [Structured Output](../structured-output.md) documentation.
...
![Multimodal Test Video](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/test.video.jpeg)
...
![Multimodal Test PNG](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/test.pdf.png)
