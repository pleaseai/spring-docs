...
![The agent loop](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/tools/spring-ai-agent-loop.png)
...
1. You define tools ([Defining Tools](#defining-tools)) and pass them to `ChatClient`.
1. `ChatClient` auto-registers a `ToolCallingAdvisor` ([The Tool Calling Loop](#the-tool-calling-loop)) that drives the loop.
1. The model decides which tools to call; `ToolCallingManager` executes them; the loop continues until the model produces a response without tool calls.
1. Other advisors in the chain — memory, observability, retries, custom logic — compose with the loop through a single dial: **advisor ordering** ([Memory and the Tool Loop](#memory-and-the-tool-loop)).

> [!NOTE]
> This architecture replaces the per-`ChatModel` tool execution loops of Spring AI 1.x. Calling `ChatModel` directly is still supported for low-level use cases — see [ChatModel Tool Calling](tools/chatmodel-tool-calling.md).

> [!TIP]
> Check the [Chat Model Comparisons](chat/comparison.md) to see which models support tool calling.
...
| --- | --- | --- |
| Declarative `@Tool` | You own the method and want minimal ceremony | [Declarative](#declarative-tool) |
| `MethodToolCallback` | You need programmatic control over a method-backed tool | [Programmatic: Method](#method-tool-callback) |
| `FunctionToolCallback` | You want to expose a `Function` / `Supplier` / `Consumer` lambda or method reference | [Programmatic: Function](#function-tool-callback) |

All three produce `ToolCallback` instances that flow through the same loop. You can mix styles freely.

<a id="declarative-tool"></a>
...
- `returnDirect` — return the result directly to the caller instead of feeding it back to the model. See [Return Direct](#return-direct).
- `resultConverter` — the `ToolCallResultConverter` to use. See [Result Conversion](#result-conversion).
...
By default all parameters are required. `@ToolParam(required = false)` or `@Nullable` makes a parameter optional. See [JSON Schema](#json-schema) for the full schema customization options.

<a id="method-tool-callback"></a>
...
The `toolObject` is required for instance methods, optional for static methods. See [Tool Specification Reference](#tool-specification-reference) for the full builder API.

<a id="function-tool-callback"></a>
...
> [!NOTE]
> Spring AI 2.0 removes the `SpringBeanToolCallbackResolver` pattern where bare `Function` beans were resolved by name via `toolNames(…​)`. Tools must now be registered as explicit `ToolCallback` beans. See [Upgrading Tool Calling from 1.x to 2.0](../upgrade-notes.md#tool-calling-1x-to-2x).
...
> [!NOTE]
> Tools registered via `ToolCallingChatOptions.toolCallbacks(…​)` on the underlying `ChatModel` **are** overridden by the request-level tool list. See [ChatModel Tool Calling](tools/chatmodel-tool-calling.md) for the precedence rules.

<a id="the-tool-calling-loop"></a>
...
![Tool Calling Advisor Flow](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/tools/spring-ai-tool-calling-advisor-flow.png)

1. The advisor sends the request to the model with all registered tool definitions included.
1. The model decides whether to call any tools and returns a response.
1. If the response contains tool calls, the advisor:

   1. Hands them to `ToolCallingManager`, which finds the matching `ToolCallback` and executes it.
   1. Appends the tool results to the conversation history.
   1. Sends the updated history back to the model.
1. Steps 2–3 repeat until the model produces a response without tool calls; that response is returned to the caller.

Both blocking (`.call()`) and streaming (`.stream()`) modes are fully supported.

`ToolCallingAdvisor` is a **recursive advisor** — it re-enters the downstream advisor chain on each iteration of the loop. The same mechanism drives structured-output validation retries and any other looped advisor pattern. See [Recursive Advisors](advisors-recursive.md) for the underlying pattern.
...
For the full `ToolCallingAdvisor` builder API, hook methods, and configuration options, see [ToolCallingAdvisor](tools/tool-calling-advisor.md).

<a id="memory-and-the-tool-loop"></a>
...
> [!NOTE]
> Disabling internal history only changes what’s forwarded to the rest of the chain each round — [tool call limits](#tool-call-limits) still see the complete history for the current turn regardless, so limits keep working correctly with a memory advisor nested inside the loop.

![ChatMemoryAdvisor inside the ToolCallingAdvisor loop](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/tools/spring-ai-tool-calling-with-inside-memory.png)
...
- [InMemoryChatMemoryRepository](chat-memory.md#_in_memory_repository)
- [RedisChatMemoryRepository](chat-memory.md#_redischatmemoryrepository)
- [Neo4jChatMemoryRepository](chat-memory.md#_neo4j_chatmemoryrepository)

Any of them is safe inside the loop.

> [!TIP]
> For JDBC-backed persistence with full tool-message support — plus event-sourced history, turn-aware compaction, and multi-agent branch isolation — see the [spring-ai-session](https://spring-ai-community.github.io/spring-ai-session/latest/) community project. It is designed for exactly this scenario and is planned for inclusion in Spring AI 2.1.

<a id="scaling-to-hundreds-of-tools"></a>
...
See [Tool Search Tool](tools/tool-search-tool.md) for the full configuration, indexing strategies, session-ID semantics, and tuning options.
...
<a id="consuming-mcp-server-tools"></a>
...
Tool callback auto-configuration is enabled by default and can be turned off with `spring.ai.mcp.client.toolcallback.enabled=false`. See [MCP Client Boot Starter](mcp/mcp-client-boot-starter-docs.md) for the full configuration surface — transports, security, filtering, name-prefix generation.

<a id="exposing-mcp-server-tools"></a>
...
Auto-configuration scans for `@McpTool`-annotated beans, generates JSON schemas for their parameters, and registers them with the MCP server. See [MCP Server Boot Starter](mcp/mcp-server-boot-starter-docs.md) for transport, security, and observability options.

<a id="combining-local-and-mcp-tools"></a>
...
![Tool Argument Augmentation](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/tools/spring-ai-tool-arg-augmenter.png)

<a id="augmentation-quick-start"></a>
...
<a id="augmentation-components"></a>
...
<a id="user-controlled-tool-execution"></a>
...
The same pattern applies to the streaming API, where each iteration’s chunk `Flux` can be forwarded to a subscriber while being aggregated with `ChatClientMessageAggregator`. See [User-Controlled Streaming](tools/tool-calling-advisor.md#user-controlled-streaming) for the full streaming variant.
...
<a id="custom-tool-advisor-autoconfig"></a>
...
For the full `ToolCallingAdvisor` builder API and hook signatures, see [ToolCallingAdvisor](tools/tool-calling-advisor.md).

<a id="tool-specification-reference"></a>
...
![Tool Context](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/tools/tool-context.jpg)
...
![Returning tool call results directly to the caller](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/tools/return-direct.jpg)
...
<a id="method-tool-limitations"></a>
...
By default, `StaticToolCallbackResolver` is auto-configured with all `ToolCallback` beans in the application context, plus tools produced by `ToolCallbackProvider` beans (with the exception of MCP providers, which are excluded to avoid eager listing — see [Consuming MCP Server Tools](#consuming-mcp-server-tools)).
...
The resolver is used internally by `ToolCallingManager` to support both [advisor-controlled execution](#the-tool-calling-loop) and [user-controlled execution](#user-controlled-tool-execution).
...
> [!WARNING]
> Enabling the resolution fallback makes every tool reachable through the resolver executable whenever the model names it, regardless of what a request attached — including risk-tier and destructive tools. Enable it only when you fully control which tools the resolver exposes.
...
See [Tool Calling Observability](../observability/index.md#_tool_calling) for the full attribute reference.

Tool call arguments and results can optionally be exported as span attributes; this is disabled by default for sensitivity reasons. See [Tool Call Arguments and Result Data](../observability/index.md#_tool_call_arguments_and_result_data) to enable it.
...
See [ChatModel Tool Calling](tools/chatmodel-tool-calling.md) for the API, precedence rules between default and runtime tools, and worked examples.
