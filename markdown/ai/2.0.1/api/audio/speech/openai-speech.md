...
> [!TIP]
> If you need to hand this stream to an API that only accepts a blocking `java.io.InputStream`, see [Consuming the Stream as a Blocking InputStream](../speech.md#streaming-consume-inputstream) for the practices that matter and a tested reference implementation.
...
- The [OpenAiAudioSpeechModelIT.java](https://github.com/spring-projects/spring-ai/blob/main/models/spring-ai-openai/src/test/java/org/springframework/ai/openai/audio/OpenAiAudioSpeechModelIT.java) test provides some general examples of how to use the library.
