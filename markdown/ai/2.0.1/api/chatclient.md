...
All `entity()` overloads accept an optional `Consumer<EntityParamSpec>` that enables two independent, composable behaviors:

- `validateSchema()` — validate the JSON response against the entity schema and automatically retry with error feedback on failure.
- `useProviderStructuredOutput()` — send the schema to the provider as an API-level constraint instead of prompt text.
...
For full details on these switches, supported providers, limitations, and the lower-level converter API, see the [Structured Output](structured-output.md) reference.
...
In the meantime, you should use the [Structured Output Converter](structured-output/converters.md) to convert the aggregated response explicitly as shown below.
...
![chat client options merging](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.1/spring-ai-docs/src/main/antora/modules/ROOT/images/chat-client-options-merging.png)
...
Use [user-controlled tool execution](tools.md#user-controlled-tool-execution) to drive the loop yourself.
...
See [User-Controlled Tool Execution — With ChatClient](tools.md#user-controlled-tool-execution) for a complete example.
...
For more details see [Extending the Loop: Custom ToolAdvisor](tools.md#extending-the-loop).
