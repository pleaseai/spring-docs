...
![Advisors API Classes](https://raw.githubusercontent.com/spring-projects/spring-ai/v1.0.0/spring-ai-docs/src/main/antora/modules/ROOT/images/advisors-api-classes.jpg)
...
![Advisors API Flow](https://raw.githubusercontent.com/spring-projects/spring-ai/v1.0.0/spring-ai-docs/src/main/antora/modules/ROOT/images/advisors-flow.jpg)
...
1. The Chat Model’s response is then passed back through the advisor chain and converted into `AdvisedResponse`. Later includes the shared `AdvisorContext` instance.
1. Each advisor can process or modify the response.
1. The final `AdvisedResponse` is returned to the client by extracting the `ChatCompletion`.
...
1. The `MessageAggregator` is a utility class that aggregates the Flux responses into a single AdvisedResponse.
...
1. The `aroundCall` method intercepts the non-streaming request and applies the Re-Reading technique.
1. The `aroundStream` method intercepts the streaming request and applies the Re-Reading technique.
1. You can control the order of execution by setting the order value. Lower values execute first.
1. Provides a unique name for the advisor.
...
![Advisors Streaming vs Non-Streaming Flow](https://raw.githubusercontent.com/spring-projects/spring-ai/v1.0.0/spring-ai-docs/src/main/antora/modules/ROOT/images/advisors-non-stream-vs-stream.jpg)
...
<a id="_backward_compatibility"></a>
