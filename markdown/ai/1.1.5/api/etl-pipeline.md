---
title: "ETL Pipeline"
source: "ROOT:api/etl-pipeline.adoc"
---
...
![Spring AI Message API](https://raw.githubusercontent.com/spring-projects/spring-ai/v1.1.5/spring-ai-docs/src/main/antora/modules/ROOT/images/spring-ai-document1-api.jpg)
...
![etl pipeline](https://raw.githubusercontent.com/spring-projects/spring-ai/v1.1.5/spring-ai-docs/src/main/antora/modules/ROOT/images/etl-pipeline.jpg)
...
<a id="etl-class-diagram"></a>

### ETL Class Diagram

The following class diagram illustrates the ETL interfaces and implementations.

![etl class diagram](https://raw.githubusercontent.com/spring-projects/spring-ai/v1.1.5/spring-ai-docs/src/main/antora/modules/ROOT/images/etl-class-diagram.jpg)

<a id="_documentreaders"></a>
...
1. `JsonReader(Resource resource, String…​ jsonKeysToUse)`
1. `JsonReader(Resource resource, JsonMetadataGenerator jsonMetadataGenerator, String…​ jsonKeysToUse)`
...
<a id="_the_getstring_pointer_method"></a>
...
<a id="_example_json_structure"></a>
...
1. `TextReader(Resource resource)`
...
<a id="_sample_document_my_page_html"></a>
...
<a id="_sample_document_code_md"></a>
...
<a id="_using_the_builder_pattern"></a>
...
<a id="_custom_punctuation_marks"></a>
...
1. `TokenTextSplitter(boolean keepSeparator)`: Creates a splitter with custom separator behavior.
1. `TokenTextSplitter(int chunkSize, int minChunkSizeChars, int minChunkLengthToEmbed, int maxNumChunks, boolean keepSeparator, List<Character> punctuationMarks)`: Full constructor with all customization options.
...
1. It splits the encoded text into chunks based on the `chunkSize`.
1. For each chunk:

   1. It decodes the chunk back into text.
   1. **Only if the total token count exceeds the chunk size**, it attempts to find a suitable break point (using the configured `punctuationMarks`) after the `minChunkSizeChars`.
   1. If a break point is found, it truncates the chunk at that point.
   1. It trims the chunk and optionally removes newline characters based on the `keepSeparator` setting.
   1. If the resulting chunk is longer than `minChunkLengthToEmbed`, it’s added to the output.
1. This process continues until all tokens are processed or `maxNumChunks` is reached.
1. Any remaining text is added as a final chunk if it’s longer than `minChunkLengthToEmbed`.
...
<a id="_contentformattransformer"></a>

### ContentFormatTransformer

Ensures uniform content formats across all documents.

<a id="_keywordmetadataenricher"></a>
...
1. `KeywordMetadataEnricher(ChatModel chatModel, PromptTemplate keywordsTemplate)`: To use a custom template for keyword extraction.
...
1. It sends this prompt to the provided `ChatModel` to generate keywords.
1. The generated keywords are added to the document’s metadata under the key "excerpt\_keywords".
1. The enriched documents are returned.

<a id="_customization"></a>
...
<a id="_summarymetadataenricher"></a>
...
1. `SummaryMetadataEnricher(ChatModel chatModel, List<SummaryType> summaryTypes, String summaryTemplate, MetadataMode metadataMode)`
...
1. It sends this prompt to the provided `ChatModel` to generate a summary.
...
1. `FileDocumentWriter(String fileName, boolean withDocumentMarkers)`
1. `FileDocumentWriter(String fileName, boolean withDocumentMarkers, MetadataMode metadataMode, boolean append)`
...
1. For each document in the input list:
   ...
   1. It writes the formatted content of the document based on the specified `metadataMode`.
1. The file is closed after all documents have been written.

<a id="_document_markers"></a>
...
See [Vector DB Documentation](vectordbs.md) for a full listing.
