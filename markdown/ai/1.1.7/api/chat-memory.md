...
1. **Azure Identity authentication**: When no key is provided, the repository uses Azure Identity (`DefaultAzureCredential`) to authenticate with managed identity, service principal, or other Azure credential sources.
...
- `PromptChatMemoryAdvisor` *(deprecated since 1.1.3; use `MessageChatMemoryAdvisor` instead)*. This advisor retrieves the conversation history and appends it to the system prompt as XML-tagged, HTML-entity-escaped blocks. Custom system prompt templates are no longer supported.
...
<a id="_promptchatmemoryadvisor"></a>

### PromptChatMemoryAdvisor

> [!IMPORTANT]
> `PromptChatMemoryAdvisor` is deprecated since 1.1.3 and will be removed in a future release.
...
<a id="_security_considerations"></a>
