...
source: "ROOT:api/embeddings/oci-genai-embeddings.adoc"
---
...
> [!TIP]
> All properties prefixed with `spring.ai.oci.genai.embedding.options` can be overridden at runtime by adding a request specific [Runtime Options](#embedding-options) to the `EmbeddingRequest` call.
