...
> [!TIP]
> All properties prefixed with `spring.ai.oci.genai.cohere.chat.options` can be overridden at runtime by adding a request specific [Runtime Options](#chat-options) to the `Prompt` call.
