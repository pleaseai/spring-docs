...
source: "ROOT:api/embeddings/vertexai-embeddings-multimodal.adoc"
---
...
> [!TIP]
> For text-only embedding use cases, we recommend using the [Vertex AI text-embeddings model](vertexai-embeddings-text.md) instead.
...
| spring.ai.vertex.ai.embedding.multimodal.options.video-interval-sec | The interval of the video the embedding will be generated. The minimum value for interval\_sec is 4. If the interval is less than 4, an InvalidArgumentError is returned. There are no limitations on the maximum value of the interval. However, if the interval is larger than min(video length, 120s), it impacts the quality of the generated embeddings. Default value: 16. | - |
