...
| spring.ai.ollama.embedding.options.model | The name of the [supported model](https://github.com/ollama/ollama?tab=readme-ov-file#model-library) to use. You can use dedicated [Embedding Model](https://ollama.com/search?c=embedding) types | mxbai-embed-large |
| spring.ai.ollama.embedding.options.keep\_alive | Controls how long the model will stay loaded into memory following the request | 5m |
...
| spring.ai.ollama.embedding.options.repeat-last-n | Sets how far back for the model to look back to prevent repetition. (Default: 64, 0 = disabled, -1 = num\_ctx) | 64 |
...
> [!TIP]
> All properties prefixed with `spring.ai.ollama.embedding.options` can be overridden at runtime by adding a request specific [Runtime Options](#embedding-options) to the `EmbeddingRequest` call.
