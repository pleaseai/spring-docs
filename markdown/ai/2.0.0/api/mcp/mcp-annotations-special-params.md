...
| Method | Description |
| --- | --- |
| `request()` | The original MCP request (e.g., `CallToolRequest`, `ReadResourceRequest`). Use `request().progressToken()` to access the progress token. |
| `exchange()` | The underlying server exchange (`McpSyncServerExchange` / `McpAsyncServerExchange`). Available in stateful mode only; `null` in stateless mode. |
| `sessionId()` | The current session identifier. |
| `clientInfo()` | The client implementation info (`Implementation`). |
| `clientCapabilities()` | The capabilities declared by the client. |
| `requestMeta()` | Metadata map from the `_meta` field of the request. Prefer this over injecting `McpMeta` when already using a context object. |
| `transportContext()` | The transport-level context (`McpTransportContext`). |
...
1. `McpSyncRequestContext` / `McpAsyncRequestContext` - Unified request context (recommended)
1. `McpSyncServerExchange` / `McpAsyncServerExchange` - Low-level server exchange context (stateful only)
1. `McpTransportContext` - Transport context for stateless operations
1. `CallToolRequest` - Full tool request for dynamic schema (tools only)
