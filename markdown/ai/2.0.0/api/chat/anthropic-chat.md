...
- **[ChatClient with ToolCallingAdvisor](../tools.md#_chatclient_tool_execution)** — the recommended approach for most use cases. `ToolCallingAdvisor` is automatically registered and manages the tool-call loop transparently.
- **[User-controlled tool execution](../tools.md#_user_controlled_tool_execution)** — use `DefaultToolCallingManager` directly when you need full control over the loop (for example, when combining tool calling with prompt caching).
...
Use this pattern when you need direct access to the `ChatModel` API — for example, when combining tool calling with [prompt caching](#_prompt_caching).
