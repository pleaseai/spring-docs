...
![Advisors Recursive](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.0/spring-ai-docs/src/main/antora/modules/ROOT/images/advisors-recursive.png)
...
Use the `.disableInternalConversationHistory()` method only when placing a memory advisor *inside* the tool-call loop (order above `ToolCallingAdvisor.DEFAULT_ORDER`). The memory advisor then handles history on every iteration. Note that only `InMemoryChatMemoryRepository` supports persisting tool-call messages; other repositories should use the default outside-loop setup above.
...
For a complete example including the streaming path, see [User-Controlled Tool Execution — With ChatClient](tools.md#_with_chatclient).

<a id="_observing_the_tool_calling_loop"></a>
...
Because `ToolCallObservingAdvisor` is at order `HIGHEST_PRECEDENCE + 400` it is inserted *after* the auto-registered `ToolCallingAdvisor` (order `HIGHEST_PRECEDENCE + 300`) in the chain, so it participates in every tool-call iteration.
...
This approach keeps the tool-calling loop fully managed by the framework while still giving you complete visibility into every intermediate step, and it pairs naturally with the `conversationHistoryEnabled` and memory advisor patterns described in [Conversation History Management](#_conversation_history_management).

<a id="_return_direct_functionality"></a>
...
1. Detect the `returnDirect` flag in the `ToolExecutionResult`
1. Break out of the tool calling loop
1. Return the tool execution result directly to the client application as a `ChatResponse` with the tool’s output as the generation content
...
Alternatively, you can enable schema validation directly on an `entity()` call without configuring the advisor manually, using [EntityParamSpec](chatclient.md#_entityparamspec):
