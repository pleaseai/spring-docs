...
| spring.ai.postgresml.embedding.vector-type | PostgresML vector type to use for the embedding. Two options are supported: `PG_ARRAY` and `PG_VECTOR`. | PG\_ARRAY |
| spring.ai.postgresml.embedding.metadata-mode | Document metadata aggregation mode | EMBED |

> [!TIP]
> All properties prefixed with `spring.ai.postgresml.embedding` can be overridden at runtime by adding a request specific [Runtime Options](#embedding-options) to the `EmbeddingRequest` call.
