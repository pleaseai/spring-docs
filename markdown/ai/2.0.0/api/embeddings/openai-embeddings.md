...
> [!TIP]
> All properties prefixed with `spring.ai.openai.embedding` can be overridden at runtime by adding a request specific [Runtime Options](#embedding-options) to the `EmbeddingRequest` call.
