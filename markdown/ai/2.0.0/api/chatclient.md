...
source: "ROOT:api/chatclient.adoc"
---
...
> [!NOTE]
> See the [Implementation Notes](#_implementation_notes) at the bottom of this document related to the combined use of imperative and reactive programming models in `ChatClient`

The fluent API has methods for building up the constituent parts of a [Prompt](prompt.md#_prompt) that is passed to the AI model as input.
...
<a id="_creating_a_chatclient"></a>

## Creating a ChatClient

The `ChatClient` is created using a `ChatClient.Builder` object.
You can obtain an autoconfigured `ChatClient.Builder` instance for any [ChatModel](chatmodel.md) Spring Boot autoconfiguration or create one programmatically.

<a id="_using_an_autoconfigured_chatclient_builder"></a>
...
<a id="_working_with_multiple_chat_models"></a>
...
<a id="_multiple_chatclients_with_a_single_model_type"></a>
...
<a id="_chatclients_for_different_model_types"></a>
...
<a id="_multiple_openai_compatible_api_endpoints"></a>
...
<a id="_chatclient_responses"></a>

## ChatClient Responses

The `ChatClient` API offers several ways to format the response from the AI Model using the fluent API.

<a id="_returning_a_chatresponse"></a>

### Returning a ChatResponse

The response from the AI model is a rich structure defined by the type `ChatResponse`.
It includes metadata about how the response was generated and can also contain multiple responses, known as [Generation](chatmodel.md#Generation)s, each with its own metadata.
...
> Notable limitations include Ollama models with reasoning/thinking mode (which may return plain text instead of JSON) and OpenAI’s lack of support for top-level array schemas.
> See [Known Limitations](structured-output-converter.md#_known_limitations) for details and workarounds.
...
<a id="_schema_validation_with_retry"></a>
...
This uses [StructuredOutputValidationAdvisor](advisors-recursive.md#structured-output-validation-advisor) internally.
...
In the meantime, you should use the [Structured Output Converter](structured-output-converter.md#StructuredOutputConverter) to convert the aggregated response explicitly as shown below.
...
By default, Spring AI uses the `StTemplateRenderer` implementation, which is based on the open-source [StringTemplate](https://www.stringtemplate.org/) engine developed by Terence Parr.
...
> It does **not** affect templates used internally by [Advisors](retrieval-augmented-generation.md#_questionansweradvisor) like `QuestionAnswerAdvisor`, which have their own template customization mechanisms (see [Custom Advisor Templates](retrieval-augmented-generation.md#_custom_template)).
...
- `entity()` to return a Java type
  ...
  - `entity(ParameterizedTypeReference<T> type, Consumer<EntityParamSpec> spec)`: like above, with optional [EntityParamSpec](#_entityparamspec) configuration.
  - `entity(Class<T> type, Consumer<EntityParamSpec> spec)`: like above, with optional [EntityParamSpec](#_entityparamspec) configuration.
  - `entity(StructuredOutputConverter<T> converter, Consumer<EntityParamSpec> spec)`: like above, with optional [EntityParamSpec](#_entityparamspec) configuration.
- `responseEntity()` to return both the `ChatResponse` and a Java type. This is useful when you need access to both the complete AI model response (with metadata and generations) and the structured output entity in a single call.
  ...
  - `responseEntity(Class<T> type, Consumer<EntityParamSpec> spec)`: like above, with optional [EntityParamSpec](#_entityparamspec) configuration.
  ...
  - `responseEntity(ParameterizedTypeReference<T> type, Consumer<EntityParamSpec> spec)`: like above, with optional [EntityParamSpec](#_entityparamspec) configuration.
  ...
  - `responseEntity(StructuredOutputConverter<T> converter, Consumer<EntityParamSpec> spec)`: like above, with optional [EntityParamSpec](#_entityparamspec) configuration.
...
<a id="_adding_metadata_to_user_messages"></a>
...
<a id="_adding_metadata_to_system_messages"></a>
...
<a id="_default_metadata_support"></a>
...
<a id="_default_system_text_with_parameters"></a>
...
- `defaultTools(Object…​ tools)`: Register one or more default tools that will be available to every request. Accepts a heterogeneous mix of `ToolCallback`, `ToolCallbackProvider`, or POJOs with `@Tool` annotated methods.
...
- `defaultTemplateRenderer(TemplateRenderer templateRenderer)`: Set a default `TemplateRenderer` for prompt templates.
- `defaultAdvisors(Advisor…​ advisor)`, `defaultAdvisors(List<Advisor> advisors)`: Advisors allow modification of the data used to create the `Prompt`.
...
![chat client options merging](https://raw.githubusercontent.com/spring-projects/spring-ai/v2.0.0/spring-ai-docs/src/main/antora/modules/ROOT/images/chat-client-options-merging.png)

<a id="_mutating_the_chatclient"></a>
...
- On `ChatClient`: `Builder mutate()` returns a `ChatClient.Builder` initialized with the client’s default settings.
- On `ChatClientRequestSpec`: `Builder mutate()` returns a `ChatClient.Builder` initialized with the request’s current settings.
...
The [Advisors API](advisors.md) provides a flexible and powerful way to intercept, modify, and enhance AI-driven interactions in your Spring applications.
...
<a id="_advisor_configuration_in_chatclient"></a>
...
> [!IMPORTANT]
> The order in which advisors are added to the chain is crucial, as it determines the sequence of their execution.
...
<a id="_retrieval_augmented_generation"></a>

### Retrieval Augmented Generation

Refer to the [Retrieval Augmented Generation](retrieval-augmented-generation.md) guide.
...
> [!TIP]
> Spring AI supports observability for LLM and vector store interactions.
> Refer to the [Observability](../observability/index.md) guide for more information.
...
The `ToolCallingAdvisor` is registered at `Ordered.HIGHEST_PRECEDENCE + 300` by default.

<a id="_disabling_auto_registration"></a>

#### Disabling Auto-Registration

<a id="_globally_all_calls"></a>
...
When this property is set, tools are still sent to the AI model as definitions, but tool calls in the response are not executed automatically.
Use [user-controlled tool execution](tools.md#_user_controlled_tool_execution) to drive the loop yourself.
...
Or you can provide your own `ToolCallingAdvisor` (or any other advisor implementing `ToolAdvisor`) — in that case auto-registration is suppressed automatically because the chain already contains a `ToolAdvisor`.
...
<a id="_customizing_the_default_toolcallingadvisor"></a>

#### Customizing the Default ToolCallingAdvisor
...
<a id="_spring_boot_advisor_order_property"></a>
...
The value must be lower than the order of any advisor that should run *inside* the tool-call loop (i.e., repeated on every iteration), and higher than any advisor that should run *outside* it (i.e., executed only once per user request).
The default is `ToolCallingAdvisor.DEFAULT_ORDER`.

<a id="_spring_boot_user_controlled_tool_execution"></a>
...
See [User-Controlled Tool Execution — With ChatClient](tools.md#_with_chatclient) for a complete example.

<a id="_spring_boot_custom_toolcallingadvisor_builder_bean"></a>
...
<a id="_tooladvisor_marker_interface"></a>

#### ToolAdvisor Marker Interface
...
For more details see [Advisor-Controlled Tool Execution](tools.md#_advisor_controlled_tool_execution_with_toolcallingadvisor).
...
For more details and usage examples, see the [Chat Memory](chat-memory.md) documentation.

<a id="_memoryadvisor_marker_interface"></a>
