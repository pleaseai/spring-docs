...
| spring.ai.embedding.transformer.onnx.modelOutputName | The ONNX model’s output node name, which we’ll use for embedding calculation. | last\_hidden\_state |
| spring.ai.embedding.transformer.onnx.gpuDeviceId | The GPU device ID to execute on. Only applicable if >= 0. Ignored otherwise.(Requires additional onnxruntime\_gpu dependency) | -1 |
...
> that means that you are using the GPU parameters `spring.ai.embedding.transformer.onnx.gpuDeviceId` , but the onnxruntime\_gpu dependency are missing.
>
> ```
> <dependency>
