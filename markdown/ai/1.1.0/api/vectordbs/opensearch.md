---
title: "OpenSearch"
source: "ROOT:api/vectordbs/opensearch.adoc"
---
...
The [OpenSearch k-NN](https://opensearch.org/docs/latest/search-plugins/knn/index/) functionality allows users to query vector embeddings from large datasets. An embedding is a numerical representation of a data object, such as text, image, audio, or document. Embeddings can be stored in the index and queried using various similarity functions.
...
- A running OpenSearch instance. The following options are available:
  ...
  - [Self-Managed OpenSearch](https://opensearch.org/docs/latest/opensearch/install/index/)
  - [Amazon OpenSearch Service](https://docs.aws.amazon.com/opensearch-service/)
- If required, an API key for the [EmbeddingModel](../embeddings.md#available-implementations) to generate the embeddings stored by the `OpenSearchVectorStore`.
...
> [!TIP]
> For both self-hosted and Amazon OpenSearch Service, use the same dependency.
...
| `spring.ai.vectorstore.opensearch.use-approximate-knn` | Whether to use approximate k-NN for faster searches. If true, uses HNSW-based approximate search. If false, uses exact brute-force k-NN. See [Approximate k-NN](https://opensearch.org/docs/latest/search-plugins/knn/approximate-knn/) and [Exact k-NN](https://opensearch.org/docs/latest/search-plugins/knn/knn-score-script/) | `false` |
...
> For example, if your OpenSearch instance is accessible at `example.com/opensearch/` instead of `example.com/`, you would set `path-prefix: /opensearch`.
...
You can leverage the generic, portable [metadata filters](../vectordbs.md#metadata-filters) with OpenSearch as well.
...
> [!NOTE]
> Those (portable) filter expressions get automatically converted into the proprietary OpenSearch [Query string query](https://opensearch.org/docs/latest/query-dsl/full-text/query-string/).
