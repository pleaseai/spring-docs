...
Generally, the query creation mechanism for JPA works as described in [Query Methods](https://docs.spring.io/spring-data/commons/reference/3.5/repositories/query-methods.html). The following example shows what a JPA query method translates into:
...
When working with large data sets, [scrolling](../repositories/query-methods-details.md#repositories.scrolling) can help to process those results efficiently without loading all results into memory.
...
1. [Offset-based scrolling](../repositories/query-methods-details.md#repositories.scrolling.offset).
This is a lighter variant than paging because it does not require the total result count.
1. [Keyset-based scrolling](../repositories/query-methods-details.md#repositories.scrolling.keyset).
...
You can add custom modifying behavior by using the custom method facilities described in [Custom Implementations for Spring Data Repositories](https://docs.spring.io/spring-data/commons/reference/3.5/repositories/custom-implementations.html).
...
> [!NOTE]
> When using Java `Stream<T>`, review the [`Stream` semantics and resource handling](../repositories/query-return-types-reference.md#return-type.stream) in the context of your JPA provider and JDBC driver considering specifically provider-specific fetch behavior and driver fetch-size settings.
