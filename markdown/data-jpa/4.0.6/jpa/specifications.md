...
The [`Specification`](https://docs.spring.io/spring-data/jpa/docs/4.0.6/api/org/springframework/data/jpa/domain/Specification.html) interface has been available for a much longer time and is tied a particular query type (select, update, delete) as per Criteria API restrictions.
...
- `stream()`: Return a `Stream<T>` to process results as a stream rather than a materialized `Collection` (see [`Stream` semantics](../repositories/query-return-types-reference.md#return-type.stream)).
