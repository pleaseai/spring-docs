...
> [!NOTE]
> It is important to note that [Class-based projections](#projections.dtos) with JPQL is limited to **constructor expressions** in your JPQL expression, e.g. `SELECT new com.example.NamesOnly(u.firstname, u.lastname) from User u`.
...
> As a workaround you may use named queries with `ResultSetMapping` or the Hibernate-specific [`ResultListTransformer`](https://docs.jboss.org/hibernate/orm/6.6/javadocs/org/hibernate/query/ResultListTransformer.html)
...
> [!NOTE]
> [Class-based projection (DTO projections)](#projections.dtos) types must declare a single constructor so that Spring Data can determine its input properties.
> If your class defines more than one constructor, then you cannot use the type without further hints for DTO projections.
> In such a case annotate the desired constructor with `@PersistenceCreator` as outlined below so that Spring Data can determine which properties to select:
>
> ```java
