---
title: "Return Values"
source: "ROOT:web/webflux/controller/ann-methods/return-types.adoc"
---

<a id="webflux-ann-return-types"></a>

# Return Values

[See equivalent in the Servlet stack](../../../webmvc/mvc-controller/ann-methods/return-types.md)

The following table shows the supported controller method return values. Note that reactive
types from libraries such as Reactor, RxJava, [or other](../../../webflux-reactive-libraries.md) are
...
| --- | --- |
| `@ResponseBody` | The return value is encoded through `HttpMessageWriter` instances and written to the response. See [`@ResponseBody`](responsebody.md). |
| `HttpEntity<B>`, `ResponseEntity<B>` | The return value specifies the full response, including HTTP headers, and the body is encoded through `HttpMessageWriter` instances and written to the response. See [`ResponseEntity`](responseentity.md). |
| `HttpHeaders` | For returning a response with headers and no body. |
| `ErrorResponse` | To render an RFC 9457 error response with details in the body, see [Error Responses](../../ann-rest-exceptions.md) |
| `ProblemDetail` | To render an RFC 9457 error response with details in the body, see [Error Responses](../../ann-rest-exceptions.md) |
| `String` | A view name to be resolved with `ViewResolver` instances and used together with the implicit model — determined through command objects and `@ModelAttribute` methods. The handler method can also programmatically enrich the model by declaring a `Model` argument (described [earlier](../../dispatcher-handler.md#webflux-viewresolution-handling)). |
| `View` | A `View` instance to use for rendering together with the implicit model — determined through command objects and `@ModelAttribute` methods. The handler method can also programmatically enrich the model by declaring a `Model` argument (described [earlier](../../dispatcher-handler.md#webflux-viewresolution-handling)). |
