...
source: "ROOT:core/aop/schema.adoc"
---
...
([@AspectJ support](ataspectj.md)) for an understanding of writing pointcut expressions and the binding
of advice parameters.

To use the aop namespace tags described in this section, you need to import the
`spring-aop` schema, as described in [XML Schema-based configuration](../appendix/xsd-schemas.md)
. See [the AOP schema](../appendix/xsd-schemas.md#aop)
...
> [auto-proxying](../aop-api/autoproxy.md) mechanism. This can cause issues (such as advice
...
language as described in [@AspectJ support](ataspectj.md). If you use the schema based declaration
...
1. References the `businessService` named pointcut defined in [Sharing Named Pointcut Definitions](ataspectj/pointcuts.md#aop-common-pointcuts).
...
the top (`<aop:config>`) level (see [Declaring a Pointcut](#aop-schema-pointcuts)).

> [!NOTE]
> As we noted in the discussion of the @AspectJ style, using *named pointcuts* can
> significantly improve the readability of your code. See [Sharing Named Pointcut Definitions](ataspectj/pointcuts.md#aop-common-pointcuts) for
> details.
...
method when it is invoked. See [Around Advice](ataspectj/advice.md#aop-ataspectj-around-advice) for notes on calling
...
<a id="aop-schema-params"></a>

### Advice Parameters

The schema-based declaration style supports fully typed advice in the same way as
described for the @AspectJ support — by matching pointcut parameters by name against
advice method parameters. See [Advice Parameters](ataspectj/advice.md#aop-ataspectj-advice-params) for details. If you wish
...
attribute in an advice annotation (as described in [Determining Argument Names](ataspectj/advice.md#aop-ataspectj-advice-params-names)).
...
1. References the `publicMethod` named pointcut defined in [Combining Pointcut Expressions](ataspectj/pointcuts.md#aop-pointcuts-combining).
...
the ordering rules are as described in [Advice Ordering](ataspectj/advice.md#aop-ataspectj-advice-ordering). The precedence
...
<a id="aop-schema-instantiation-models"></a>
...
[Advice Types in Spring](../aop-api/advice.md#aop-api-advice-types). Advisors can take advantage of AspectJ pointcut expressions.
...
[An AOP Example](ataspectj/example.md) looks when rewritten with the schema support.
