---
title: "Exceptions"
source: "ROOT:web/webmvc/mvc-controller/ann-exceptionhandler.adoc"
---
...
[See equivalent in the Reactive stack](../../webflux/controller/ann-exceptions.md)
...
<a id="mvc-ann-exceptionhandler-exc"></a>
...
level, [HandlerExceptionResolver](../mvc-servlet/exceptionhandlers.md) mechanism.

<a id="mvc-ann-exceptionhandler-media"></a>

## Media Type Mapping

[See equivalent in the Reactive stack](../../webflux/controller/ann-exceptions.md#webflux-ann-exceptionhandler-media)
...
[See equivalent in the Reactive stack](../../webflux/controller/ann-exceptions.md#webflux-ann-exceptionhandler-args)
...
| `RedirectAttributes` | Specify attributes to use in case of a redirect — (that is to be appended to the query string) and flash attributes to be stored temporarily until the request after the redirect. See [Redirect Attributes](ann-methods/redirecting-passing-data.md) and [Flash Attributes](ann-methods/flash-attributes.md). |
| `@SessionAttribute` | For access to any session attribute, in contrast to model attributes stored in the session as a result of a class-level `@SessionAttributes` declaration. See [`@SessionAttribute`](ann-methods/sessionattribute.md) for more details. |
| `@RequestAttribute` | For access to request attributes. See [`@RequestAttribute`](ann-methods/requestattrib.md) for more details. |

<a id="mvc-ann-exceptionhandler-return-values"></a>

## Return Values

[See equivalent in the Reactive stack](../../webflux/controller/ann-exceptions.md#webflux-ann-exceptionhandler-return-values)

`@ExceptionHandler` methods support the following return values:

| Return value | Description |
| --- | --- |
| `@ResponseBody` | The return value is converted through `HttpMessageConverter` instances and written to the response. See [`@ResponseBody`](ann-methods/responsebody.md). |
| `HttpEntity<B>`, `ResponseEntity<B>` | The return value specifies that the full response (including the HTTP headers and the body) be converted through `HttpMessageConverter` instances and written to the response. See [ResponseEntity](ann-methods/responseentity.md). |
| `ErrorResponse` | To render an RFC 9457 error response with details in the body, see [Error Responses](../mvc-ann-rest-exceptions.md) |
| `ProblemDetail` | To render an RFC 9457 error response with details in the body, see [Error Responses](../mvc-ann-rest-exceptions.md) |
