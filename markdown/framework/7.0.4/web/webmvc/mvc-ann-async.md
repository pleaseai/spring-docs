...
- [`DeferredResult`](#mvc-ann-async-deferredresult),
[`Callable`](#mvc-ann-async-callable), and
[`WebAsyncTask`](#mvc-ann-async-webasynctask) return values
...
[SSE](#mvc-ann-async-sse) and
[raw data](#mvc-ann-async-output-stream).
...
<a id="mvc-ann-async-webasynctask"></a>

## `WebAsyncTask`

`WebAsyncTask` is comparable to using [Callable](#mvc-ann-async-callable)
...
[`AsyncHandlerInterceptor`](https://docs.spring.io/spring-framework/docs/7.0.4/javadoc-api/org/springframework/web/servlet/AsyncHandlerInterceptor.html)
for more details.

`DeferredResult` provides `onTimeout(Runnable)` and `onCompletion(Runnable)` callbacks.
See the [javadoc of `DeferredResult`](https://docs.spring.io/spring-framework/docs/7.0.4/javadoc-api/org/springframework/web/context/request/async/DeferredResult.html)
...
[Reactive Libraries](../webflux-reactive-libraries.md) in the WebFlux section).
...
> [`ReactiveAdapterRegistry`](https://docs.spring.io/spring-framework/docs/7.0.4/javadoc-api/org/springframework/core/ReactiveAdapterRegistry.html) from
