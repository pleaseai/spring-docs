...
source: "ROOT:core/aop/ataspectj/pointcuts.adoc"
---
...
> class, consider the use of Spring-driven [native AspectJ weaving](../using-aspectj.md#aop-aj-ltw) instead
...
1. `inTrading` matches if a method execution is in the trading module.
1. `tradingOperation` matches if a method execution represents any public method in the
trading module.
...
The `<aop:config>` and `<aop:advisor>` elements are discussed in [Schema-based AOP Support](../schema.md). The
transaction elements are discussed in [Transaction Management](../../../data-access/transaction.md).
...
`AccountService` interface:
  ...
  > [!NOTE]
  > `this` is more commonly used in a binding form. See the section on [Declaring Advice](advice.md)
...
implements the `AccountService` interface:
  ...
  > [!NOTE]
  > `target` is more commonly used in a binding form. See the [Declaring Advice](advice.md) section
...
and where the argument passed at runtime is `Serializable`:
  ...
  > [!NOTE]
  > `args` is more commonly used in a binding form. See the [Declaring Advice](advice.md) section
...
`@Transactional` annotation:
  ...
  > [!NOTE]
  > You can also use `@target` in a binding form. See the [Declaring Advice](advice.md) section for
...
target object has an `@Transactional` annotation:
  ...
  > [!NOTE]
  > You can also use `@within` in a binding form. See the [Declaring Advice](advice.md) section for
  > how to make the annotation object available in the advice body.
...
`@Transactional` annotation:
  ...
  > [!NOTE]
  > You can also use `@annotation` in a binding form. See the [Declaring Advice](advice.md) section
...
and where the runtime type of the argument passed has the `@Classified` annotation:
  ...
  > [!NOTE]
  > You can also use `@args` in a binding form. See the [Declaring Advice](advice.md) section
