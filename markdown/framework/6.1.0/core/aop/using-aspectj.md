...
source: "ROOT:core/aop/using-aspectj.adoc"
---
...
to use the aspects in it. [Using AspectJ to Dependency Inject Domain Objects with Spring](#aop-atconfigurable) and [Other Spring aspects for AspectJ](#aop-ajlib-other) discuss the
content of this library and how you can use it. [Configuring AspectJ Aspects by Using Spring IoC](#aop-aj-configure) discusses how to
dependency inject AspectJ aspects that are woven using the AspectJ compiler. Finally,
[Load-time Weaving with AspectJ in the Spring Framework](#aop-aj-ltw) provides an introduction to load-time weaving for Spring applications
that use AspectJ.
...
at the field or method level (see [Annotation-based Container Configuration](../beans/annotation-config.md) for further details).
...
[readResolve()](https://docs.oracle.com/javase/8/docs/api/java/io/Serializable.html)).
...
> Programming Guide](https://www.eclipse.org/aspectj/doc/next/progguide/index.html).

For this to work, the annotated types must be woven with the AspectJ weaver. You can
either use a build-time Ant or Maven task to do this (see, for example, the
[AspectJ Development
Environment Guide](https://www.eclipse.org/aspectj/doc/released/devguide/antTasks.html)) or load-time weaving (see [Load-time Weaving with AspectJ in the Spring Framework](#aop-aj-ltw)). The
...
[`context` namespace](../appendix/xsd-schemas.md#context)
...
Further, [in certain environments](#aop-aj-ltw-environments), this support enables
...
[Petclinic sample application](https://github.com/spring-projects/spring-petclinic).
...
> use @AspectJ with [Java configuration](../beans/java.md). Specifically, you can use the
> `@EnableLoadTimeWeaving` annotation as an alternative to `<context:load-time-weaver/>`
> (see [below](#aop-aj-ltw-spring) for details).
...
[agents
to instrument programs that run on the JVM](https://docs.oracle.com/javase/8/docs/api/java/lang/instrument/package-summary.html). The Spring Framework ships with such an
...
If you use the [Spring-provided agent to enable instrumentation](#aop-aj-ltw-environments-generic)
, you also need:
...
<a id="aop-aj-ltw-spring-env-impls"></a>
...
<a id="aop-aj-ltw-ltw-tag-attrs"></a>
...
<a id="aop-aj-ltw-environments-tomcat-jboss-etc"></a>
...
<a id="aop-aj-ltw-environments-generic"></a>
