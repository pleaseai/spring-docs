...
source: "ROOT:core/beans/factory-nature.adoc"
---
...
- [Lifecycle Callbacks](#beans-factory-lifecycle)
...
> For details, see [Using `@PostConstruct` and `@PreDestroy`](annotation-config/postconstruct-and-predestroy-annotations.md).
>
...
[Container Extension Points](factory-extension.md).
...
the [`@PostConstruct`](annotation-config/postconstruct-and-predestroy-annotations.md) annotation or
...
`@Bean`. See [Receiving Lifecycle Callbacks](java/bean-annotation.md#beans-java-lifecycle-callbacks). Consider the following example:
...
unnecessarily couples the code to Spring. Alternatively, we suggest using
the [`@PreDestroy`](annotation-config/postconstruct-and-predestroy-annotations.md) annotation or
...
[Receiving Lifecycle Callbacks](java/bean-annotation.md#beans-java-lifecycle-callbacks). Consider the following definition:
...
with the standard lifecycle callback contract [described previously](#beans-factory-lifecycle)).
...
- The [`InitializingBean`](#beans-factory-lifecycle-initializingbean) and
[`DisposableBean`](#beans-factory-lifecycle-disposablebean) callback interfaces
- Custom `init()` and `destroy()` methods
- The [`@PostConstruct` and `@PreDestroy` annotations](annotation-config/postconstruct-and-predestroy-annotations.md)
...
> [preceding section](#beans-factory-lifecycle-default-init-destroy-methods).
...
<a id="beans-factory-thread-safety"></a>
...
[Additional Capabilities of the `ApplicationContext`](context-introduction.md).
...
(as described in [Autowiring Collaborators](dependencies/factory-autowire.md)) can provide a dependency of type
...
[Using `@Autowired`](annotation-config/autowired.md).
...
<a id="beans-factory-nature-aware-list"></a>
...
| `ApplicationEventPublisherAware` | Event publisher of the enclosing `ApplicationContext`. | [Additional Capabilities of the `ApplicationContext`](context-introduction.md) |
| `BeanClassLoaderAware` | Class loader used to load the bean classes. | [Instantiating Beans](definition.md#beans-factory-class) |
| `BeanFactoryAware` | Declaring `BeanFactory`. | [The `BeanFactory` API](beanfactory.md) |
| `BeanNameAware` | Name of the declaring bean. | [`ApplicationContextAware` and `BeanNameAware`](#beans-factory-aware) |
| `LoadTimeWeaverAware` | Defined weaver for processing class definition at load time. | [Load-time Weaving with AspectJ in the Spring Framework](../aop/using-aspectj.md#aop-aj-ltw) |
| `MessageSourceAware` | Configured strategy for resolving messages (with support for parameterization and internationalization). | [Additional Capabilities of the `ApplicationContext`](context-introduction.md) |
| `NotificationPublisherAware` | Spring JMX notification publisher. | [Notifications](../../integration/jmx/notifications.md) |
| `ResourceLoaderAware` | Configured loader for low-level access to resources. | [Resources](../../web/webflux-webclient/client-builder.md#webflux-client-builder-reactor-resources) |
| `ServletConfigAware` | Current `ServletConfig` the container runs in. Valid only in a web-aware Spring `ApplicationContext`. | [Spring MVC](../../web/webmvc.md#mvc) |
| `ServletContextAware` | Current `ServletContext` the container runs in. Valid only in a web-aware Spring `ApplicationContext`. | [Spring MVC](../../web/webmvc.md#mvc) |
