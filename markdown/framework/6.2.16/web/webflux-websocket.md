---
title: "WebSockets"
source: "ROOT:web/webflux-websocket.adoc"
---

<a id="webflux-websocket"></a>

# WebSockets

[See equivalent in the Servlet stack](websocket.md)

This part of the reference documentation covers support for reactive-stack WebSocket
messaging.

<a id="introduction-to-websocket"></a>
...
1. Using the `Upgrade` connection.
...
<a id="webflux-websocket-server"></a>

## WebSocket API

[See equivalent in the Servlet stack](websocket/stomp/server-config.md)
...
<a id="webflux-websocket-server-handler"></a>

### Server

[See equivalent in the Servlet stack](websocket/server.md#websocket-server-handler)
...
If using the [WebFlux Config](webflux/dispatcher-handler.md#webflux-framework-config) there is nothing
...
<a id="webflux-websockethandler"></a>
...
1. Do something with each message.
1. Perform nested asynchronous operations that use the message content.
1. Return a `Mono<Void>` that completes when receiving completes.
...
> [Data Buffers and Codecs](../core/databuffer-codec.md).
...
1. Create the outbound message, producing a combined flow.
1. Return a `Mono<Void>` that does not complete while we continue to receive.
...
1. Join the streams and return a `Mono<Void>` that completes when either stream ends.
...
<a id="webflux-websocket-databuffer"></a>
...
<a id="webflux-websocket-server-handshake"></a>

### Handshake

[See equivalent in the Servlet stack](websocket/server.md#websocket-server-handshake)
...
<a id="webflux-websocket-server-config"></a>

### Server Configuration

[See equivalent in the Servlet stack](websocket/server.md#websocket-server-runtime-configuration)
...
[WebFlux Config](webflux/config.md#webflux-config-websocket-service), or otherwise if
...
<a id="webflux-websocket-server-cors"></a>

### CORS

[See equivalent in the Servlet stack](websocket/server.md#websocket-server-allowed-origins)
...
<a id="webflux-websocket-client"></a>
