...
source: "ROOT:web/webflux-functional.adoc"
---
...
[See equivalent in the Servlet stack](webmvc-functional.md)
...
the same [Reactive Core](webflux/reactive-spring.md) foundation.

<a id="webflux-fn-overview"></a>

## Overview

[See equivalent in the Servlet stack](webmvc-functional.md#webmvc-fn-overview)
...
through one of the built-in [server adapters](webflux/reactive-spring.md#webflux-httphandler):
...
<a id="webflux-fn-handler-functions"></a>

## HandlerFunction

[See equivalent in the Servlet stack](webmvc-functional.md#webmvc-fn-handler-functions)
...
For more on that, see [Reactive Libraries](../web-reactive.md#webflux-reactive-libraries).

<a id="webflux-fn-request"></a>
...
<a id="webflux-fn-handler-classes"></a>
...
JSON.
1. `createPerson` is a handler function that stores a new `Person` contained in the request body.
...
1. `getPerson` is a handler function that returns a single person, identified by the `id` path
...
<a id="webflux-fn-handler-validation"></a>

### Validation

A functional endpoint can use Spring’s [validation facilities](webmvc/mvc-config/validation.md) to
apply validation to the request body. For example, given a custom Spring
[Validator](webmvc/mvc-config/validation.md) implementation for a `Person`:
...
1. Apply validation.
1. Raise exception for a 400 response.
...
See [Spring Validation](../core/validation/beanvalidation.md).

<a id="webflux-fn-router-functions"></a>

## `RouterFunction`

[See equivalent in the Servlet stack](webmvc-functional.md#webmvc-fn-router-functions)
...
1. `GET /person` with an `Accept` header that matches JSON is routed to
`PersonHandler.listPeople`
1. `POST /person` with no additional predicates is mapped to
`PersonHandler.createPerson`, and
1. `otherRoute` is a router function that is created elsewhere, and added to the route built.
...
[WebFlux Config](webflux/config.md#webflux-config-api-version), and then you can
...
[supported version](webmvc/mvc-config/api-version.md).

See [API Versioning](webflux-versioning.md) for more details on underlying
infrastructure and support for API Versioning.

<a id="webflux-fn-serving-resources"></a>
...
> [`RouterFunctions#resource(java.util.function.Function)`](<https://docs.spring.io/spring-framework/docs/7.0.1/javadoc-api/org/springframework/web/reactive/function/server/RouterFunctions.html#resources(java.util.function.Function)>).

<a id="webflux-fn-resource"></a>
...
<a id="webflux-fn-running"></a>

## Running a Server

[See equivalent in the Servlet stack](webmvc-functional.md#webmvc-fn-running)
...
[HttpHandler](webflux/reactive-spring.md#webflux-httphandler) for server-specific instructions.

A more typical option, also used by Spring Boot, is to run with a
[`DispatcherHandler`](webflux/dispatcher-handler.md)-based setup through the
[WebFlux Config](webflux/dispatcher-handler.md#webflux-framework-config), which uses Spring configuration to declare the
...
configuration, [orders them](../core/beans/annotation-config/autowired.md#beans-factory-ordered), combines them through
...
[DispatcherHandler](webflux/dispatcher-handler.md) for how to run it):
...
<a id="webflux-fn-handler-filter-function"></a>

## Filtering Handler Functions

[See equivalent in the Servlet stack](webmvc-functional.md#webmvc-fn-handler-filter-function)
...
1. The `after` filter that logs the response is applied to all routes, including the nested ones.
...
> [`CorsWebFilter`](webflux-cors.md#webflux-cors-webfilter).
