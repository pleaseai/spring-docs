---
title: "Data Buffers and Codecs"
source: "ROOT:core/databuffer-codec.adoc"
---
...
- [`DataBufferFactory`](#databuffers-factory) abstracts the creation of a data buffer.
- [`DataBuffer`](#databuffers-buffer) represents a byte buffer, which may be
[pooled](#databuffers-buffer-pooled).
- [`DataBufferUtils`](#databuffers-utils) offers utility methods for data buffers.
...
1. Wrap an existing `byte[]` or `java.nio.ByteBuffer`, which decorates the given data with
...
- Pooled buffers and reference counting via [`PooledDataBuffer`](#databuffers-buffer-pooled).
...
[Codecs](../web/webflux/reactive-spring.md#webflux-codecs) in the WebFlux section.
...
1. If a `Decoder` is using `Flux` or `Mono` operators such as `flatMap`, `reduce`, and
...
1. If a `Decoder` holds on to one or more data buffers in any other way, it must
