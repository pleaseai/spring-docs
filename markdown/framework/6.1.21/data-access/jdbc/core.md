---
title: "Using the JDBC Core Classes to Control Basic JDBC Processing and Error Handling"
source: "ROOT:data-access/jdbc/core.adoc"
---
...
- [Using `JdbcTemplate`](#jdbc-JdbcTemplate)
- [Using `NamedParameterJdbcTemplate`](#jdbc-NamedParameterJdbcTemplate)
- [Unified JDBC Query/Update Operations: `JdbcClient`](#jdbc-JdbcClient)
- [Using `SQLExceptionTranslator`](#jdbc-SQLExceptionTranslator)
- [Running Statements](#jdbc-statements-executing)
- [Running Queries](#jdbc-statements-querying)
- [Updating the Database](#jdbc-updates)
- [Retrieving Auto-generated Keys](#jdbc-auto-generated-keys)

<a id="jdbc-JdbcTemplate"></a>
...
hierarchy defined in the `org.springframework.dao` package. (See [Consistent Exception Hierarchy](../dao.md#dao-exceptions).)
...
See the attendant [javadoc](https://docs.spring.io/spring-framework/docs/6.1.21/javadoc-api/org/springframework/jdbc/core/JdbcTemplate.html) for that.

<a id="jdbc-JdbcTemplate-examples-query"></a>
...
<a id="jdbc-JdbcTemplate-examples-update"></a>
...
<a id="jdbc-JdbcTemplate-examples-other"></a>
...
More sophisticated stored procedure support is [covered later](object.md#jdbc-StoredProcedure).

<a id="jdbc-JdbcTemplate-idioms"></a>
...
[`NamedParameterJdbcTemplate`](#jdbc-NamedParameterJdbcTemplate) class) is to
...
1. Annotate the `DataSource` setter method with `@Autowired`.
1. Create a new `JdbcTemplate` with the `DataSource`.
...
1. Annotate the class with `@Repository`.
1. Constructor injection of the `DataSource`.
1. Create a new `JdbcTemplate` with the `DataSource`.
...
<a id="jdbc-NamedParameterJdbcTemplate"></a>
...
<a id="jdbc-SQLExceptionTranslator"></a>
...
1. Any custom implementation of the `SQLExceptionTranslator` interface that is provided
as the `customSqlExceptionTranslator` property of the `SQLErrorCodes` class.
1. The list of instances of the `CustomSQLErrorCodesTranslation` class (provided for the
`customTranslations` property of the `SQLErrorCodes` class) are searched for a match.
1. Error code matching is applied.
1. Use the fallback translator. `SQLExceptionSubclassTranslator` is the default fallback
...
<a id="jdbc-statements-executing"></a>
...
<a id="jdbc-statements-querying"></a>
...
<a id="jdbc-auto-generated-keys"></a>
