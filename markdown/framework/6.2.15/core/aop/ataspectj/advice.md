...
source: "ROOT:core/aop/ataspectj/advice.adoc"
---
...
pointcut* or a reference to a [*named pointcut*](pointcuts.md#aop-common-pointcuts).
...
If we use a [named pointcut](pointcuts.md#aop-common-pointcuts), we can rewrite the preceding example
as follows:
...
> [following section on advice parameters](#aop-ataspectj-advice-proceeding-with-the-call).
...
1. References the `publicMethod` named pointcut defined in [Combining Pointcut Expressions](pointcuts.md#aop-pointcuts-combining).
...
pointcut annotation. See [Explicit Argument Names](#aop-ataspectj-advice-params-names-explicit) for details.
...
[javadoc](https://docs.spring.io/spring-framework/docs/6.2.15/javadoc-api/org/springframework/aop/aspectj/AspectJAdviceParameterNameDiscoverer.html)
for details on the algorithm used.

<a id="aop-ataspectj-advice-params-names-explicit"></a>
...
1. References the `publicMethod` named pointcut defined in [Combining Pointcut Expressions](pointcuts.md#aop-pointcuts-combining).
1. Declares `bean` and `auditable` as the argument names.
...
1. References the `inDataAccessLayer` named pointcut defined in [Sharing Named Pointcut Definitions](pointcuts.md#aop-common-pointcuts).
