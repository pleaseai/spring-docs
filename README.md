...
| **Buildable versions** | Spring Boot `3.3.0`+, Spring Framework `6.1.0`+, Spring Security `6.2.0`+, Spring AI `1.0.0`+, and eleven Spring Data stores (project key in parentheses): JPA `3.2.0`+ (`data-jpa`), Cassandra `4.2.0`+ (`data-cassandra`), Couchbase `5.2.0`+ (`data-couchbase`), Elasticsearch `5.2.0`+ (`data-elasticsearch`), KeyValue `3.2.0`+ (`data-keyvalue`), LDAP `3.2.0`+ (`data-ldap`), MongoDB `4.2.0`+ (`data-mongodb`), Neo4j `7.2.0`+ (`data-neo4j`), Redis `3.2.0`+ (`data-redis`), Relational `3.2.0`+ (`data-relational`) and REST `4.2.0`+ (`data-rest`). Boot 3.2 and older predate the Antora component entirely; 4.0.0-4.0.7 moved to the 4.x layout but published no content archive, so they are reconstructed from the tag like 3.3-3.x rather than downloaded, and 4.1.0 is tagged with no archive published yet. Spring AI 0.8.x shares the 1.x layout, but its artifacts never reached Maven Central, so no consumer can pin a dependency to what those docs describe. Each Spring Data store's versions before the 2023.1 release train ship no Antora component. |
